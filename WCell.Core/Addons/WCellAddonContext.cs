...
namespace WCell.Core.Addons
{
  ...
  public class WCellAddonContext
  {
    ...
    public void InitAddon()
    {
      ...
      {
        if(type.GetInterfaces().Contains(typeof(IWCellAddon)))
        {
          WCellAddonAttribute[] customAttributes = (WCellAddonAttribute[])type.GetCustomAttributes<WCellAddonAttribute>(true);
          m_attr = customAttributes.Length > 0 ? customAttributes[0] : null;
