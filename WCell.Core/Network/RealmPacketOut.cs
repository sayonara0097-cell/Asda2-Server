...
namespace WCell.Core.Network
{
  ...
  public class RealmPacketOut : PacketOut
  {
    ...
    public void FinalizeAsda(bool addEndOfPacket, Locale locale)
    {
      ...
      EncodedLocale = locale;
            List<RealmServerOpCode> array = new List<RealmServerOpCode> { RealmServerOpCode.EndMove, RealmServerOpCode.StartMove, RealmServerOpCode.EndMoveCommon, RealmServerOpCode.StartMoveCommon, RealmServerOpCode.MonstMove };

            if (!array.Contains(OpCode))
            {
