...
namespace WCell.RealmServer.Guilds
{
  ...
  public sealed class GuildMgr : Manager<GuildMgr>
  {
    ...
    internal void OnCharacterLogin(Character chr)
    {
      ...
      if(guildMember == null && chr.Record.GuildId != 0U)
      {
        Guild guild = GetGuild(chr.Record.GuildId);
        if(guild != null && guild.Members.TryGetValue(chr.EntityId.Low, out guildMember))
          guildMember.Character = chr;
      }
    ...
    internal void OnCharacterLogout(GuildMember member)
    {
      ...
      if(character == null)
      {
        ...
        {
          using(membersLock.EnterWriteLock())
            OfflineMembers[member.Id] = member;
        }
