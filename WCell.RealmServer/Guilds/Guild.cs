...
namespace WCell.RealmServer.Guilds
{
    ...
    IGenericChatTarget
  {
    ...
    {
        get { return _maxMembersCount; }
        set
        {
            _maxMembersCount = value;
        }
    }
      ...
      : this(true)
    {
      ...
      if(m_leader == null)
        throw new InvalidOperationException("Could not create guild leader member.");
      Character leaderCharacter = World.GetCharacter(leader.Name, true);
      if(leaderCharacter != null)
        Leader.Character = leaderCharacter;
      m_leader.RankId = 0;
    ...
    public GuildMember AddMember(CharacterRecord chr)
    {
      ...
      lock(this)
      {
        ...
        GuildMember existingMember = GuildMember.FindAllByProperty("CharacterLowId",
          (long) chr.EntityLowId).FirstOrDefault();
        if(existingMember != null)
        {
          if(existingMember.GuildId != Id)
            return null;
          guildMember = existingMember;
          guildMember.Init(this, characterByAccId);
        }
        else
        {
          guildMember = new GuildMember(chr, this, m_ranks.Last());
          if(characterByAccId != null)
            guildMember.Character = characterByAccId;
          guildMember.Create();
        }
    ...
    protected void OnRemoveMember(GuildMember member)
    {
      ...
      CharacterRecord record = character == null ? CharacterRecord.GetRecord(member.Id) : character.Record;
      if(record != null)
      {
        record.GuildId = 0U;
        record.SaveAndFlush();
    ...
    public LearnGuildSkillResult TryLearnSkill(GuildSkillId skillId, out GuildSkill skill)
    {
      ...
      if(skill != null)
      {
        ...
        if(skill.IsActivated)
          return LearnGuildSkillResult.CantLevelupCurrentActivatedSkills;
        ...
        Asda2GuildHandler.SendGuildSkillsInfoToGuild(this);
      }
      else
      {
        GuildSkillTemplate template = GuildSkillTemplate.Templates[(int) skillId];
        if(template == null || template.LearnCosts.Length <= 1)
          return LearnGuildSkillResult.IncifitientPoints;
        if(!SubstractGuildPoints(template.LearnCosts[1]))
          return LearnGuildSkillResult.IncifitientPoints;
    ...
    private void ImpeachmentCallback()
    {
      ...
      float num = (float) (_acceptedMembers.Count / (double) (MemberCount) * 100.0);
    ...
    public bool LevelUp()
    {
      if(Level <= 0 || Level >= CharacterFormulas.GuildLevelUpCost.Length)
        return false;
