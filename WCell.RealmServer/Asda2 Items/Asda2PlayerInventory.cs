...
namespace WCell.RealmServer.Asda2_Items
{
    public class Asda2PlayerInventory
    {
        ...
        public short Weight
        {
            ...
            {
                int w = 0;
                RegularItems.Where((e) => e != null).ForEach((item) => w += (item.Record.Weight * item.Amount));
                ShopItems.Where((e) => e != null).ForEach((item) => w += (item.Record.Weight * item.Amount));
                return (short)w;
            }
        ...
        public double WeightPrc
        {
            get
            {
                ...
                int denominator = Owner.Record.MaxWeight;
                double result = 0.0;
          ...
          ref short destSlot)
        {
            ...
            {
                if (destInv == Asda2InventoryType.Shop)
                    destSlot = FindFreeShopItemsSlot();
                else if (destInv == Asda2InventoryType.Regular)
                    destSlot = FindFreeRegularItemsSlot();
            ...
            if (destInv == Asda2InventoryType.Equipment && destSlot == 9 && asda2Item1 != null && (asda2Item1.Category == Asda2ItemCategory.Bow || asda2Item1.Category == Asda2ItemCategory.Crossbow || asda2Item1.Category == Asda2ItemCategory.Ballista))
            {
                Asda2Item arrows = asda2Item1.Category == Asda2ItemCategory.Bow ? FindRegularItem(20565) : asda2Item1.Category == Asda2ItemCategory.Crossbow ? FindRegularItem(20568) : FindRegularItem(20571);
                short des = 10;
                if(arrows != null)
                TrySwap(Asda2InventoryType.Regular, arrows.Slot, Asda2InventoryType.Equipment, ref des);
            }
            return status;
        }
          ...
          Asda2InventoryType? requiredInventoryType = null, Asda2Item itemToCopyStats = null)
        {
            ...
            if (template.IsStackable)
            {
                ...
                {
                    item.Amount += amount;
                    if (itemToCopyStats != null && itemToCopyStats.Record != null && itemToCopyStats.IsSoulbound)
            ...
            {
                item.Weight = itemToCopyStats.Weight;
                item.Enchant = itemToCopyStats.Enchant;
                item.EnchantResetCount = itemToCopyStats.EnchantResetCount;
                item.SealCount = itemToCopyStats.SealCount;
        ...
        private UseItemResult UseItemUnchecked(Asda2Item item)
        {
            switch (item.Category)
            {
                ...
                case Asda2ItemCategory.ResurectScroll:
                    ...
                    Asda2CharacterHandler.SendResurrectOptionsResponse(target.Client, Owner, Asda2CharacterHandler.ResurrectOptions.HelpFrom);
                    break;
        ...
        {
            if(item.Template.Category == Asda2ItemCategory.PetExp)
            {
                return true;
            }
        ...
        public void BuyItems(List<ItemStub> itemStubs)
        {
            ...
            if (!CheckFreeRegularItemsSlots(CountRequiredBuySlots(asda2ItemTemplateList,
                   Asda2InventoryType.Regular)) ||
                !CheckShopItemsSlots(CountRequiredBuySlots(asda2ItemTemplateList, Asda2InventoryType.Shop)))
            {
            ...
            else
            {
                ...
                else
                {
                    for (int index1 = 0; index1 < 7; ++index1)
                    {
                        ...
                        {
                            int amount = Math.Max(1, itemStubs[index1].Amount);
        ...
        private int CountRequiredBuySlots(IEnumerable<Asda2ItemTemplate> templates, Asda2InventoryType inventoryType)
        {
            int count = 0;
            HashSet<int> stackableItemsWithoutSlot = new HashSet<int>();
            foreach (Asda2ItemTemplate template in templates)
            {
                if (template == null || CalcIntentoryTypeToAdd(new Asda2InventoryType?(), template) != inventoryType)
                    continue;
                if (template.IsStackable)
                {
                    if (FindItem(template, new Asda2InventoryType?(inventoryType)) != null)
                        continue;
                    if (!stackableItemsWithoutSlot.Add((int)template.ItemId))
                        continue;
                }
        ...
        public void TryPickUpItem(short x, short y)
        {
            ...
            else if (!CanPickUpProtectedLoot(lootItem))
            {
                ...
                progressRecord.SaveAndFlush();
                Asda2InventoryHandler.SendLootPickupFailedResponse(lootItem, Owner, false,
                  Asda2PickUpItemStatus.ProtectedLoot);
            }
            else
            {
                if (Owner.IsInGroup && Owner.Group.LootMethod != Constants.Looting.LootMethod.MasterLoot)
                {
                    Owner.Map.ClearLootSlot(x, y);
                    Owner.Group.GetNearbyLooters(lootItem.Loot.Lootable, (WorldObject) Owner,
                        (ICollection<Asda2LooterEntry>)lootItem.Loot.Looters);
                    Random random = new Random();

                    if (IsMoneyLootItem(lootItem))
                    {
                        if (lootItem.Loot.Looters.Count == 0)
                        {
                            GiveLootMoney(Owner, GetMoneyAmount(lootItem));
                        }
                        else if (lootItem.Amount > 1)
                        {
                            foreach (var looter in lootItem.Loot.Looters)
                            {
                                GiveLootMoney(looter.Owner, GetGroupMoneyShare(lootItem));
                            }
                        }
                        else
                        {
                            int nextLooterIndex = (lastLooterIndex + 1) % lootItem.Loot.Looters.Count;
                            var looter = lootItem.Loot.Looters[nextLooterIndex];
                            lastLooterIndex = nextLooterIndex;
                            GiveLootMoney(looter.Owner, GetMoneyAmount(lootItem));
                        }

                        CompleteLootPickup(lootItem);
                        return;
                    }
                    
                    if(lootItem.Amount > 1)
                    {
                        foreach (var looter in lootItem.Loot.Looters)
                        {
                            int div = random.Next(1, lootItem.Amount);
                            if(lootItem.Template.Id == 20551)
                            {
                                looter.Owner.AddMoney((uint)((lootItem.Amount / lootItem.Loot.Looters.Count) < 1 ? 1 : (lootItem.Amount / lootItem.Loot.Looters.Count)));
                                Asda2InventoryHandler.SendGoldPickupedResponse(looter.Owner.Money, looter.Owner);
                            }
                            else
                            {
                                Asda2Item asda2Item = null;
                                Asda2InventoryError asda2InventoryError = looter.Owner.Asda2Inventory.TryAdd((int)lootItem.Template.ItemId, Owner.Group.LootMethod == Constants.Looting.LootMethod.GroupLoot ? (lootItem.Amount / lootItem.Loot.Looters.Count) < 1 ? 1 : (lootItem.Amount / lootItem.Loot.Looters.Count) : lootItem.Amount / div,
                                ...
                                else
                                {
                                    if (asda2Item != null && asda2Item.Template.Quality >= Asda2ItemQuality.Green)
                                        ChatMgr.SendGlobalMessageResponse(looter.Owner.Name, ChatMgr.Asda2GlobalMessageType.HasObinedItem,
                                            asda2Item.ItemId, 0, 0);
                                    switch (++looter.Owner.Achievements.GetOrCreateProgressRecord(100U).Counter)
                                    {
                                        case 500:
                                            looter.Owner.DiscoverTitle(Asda2TitleId.Gatherer233);
                                            break;
                                        case 1000:
                                            looter.Owner.GetTitle(Asda2TitleId.Gatherer233);
                                            break;
                                    }

                                    Asda2InventoryHandler.SendItemPickupedResponse(Asda2PickUpItemStatus.Ok, asda2Item, looter.Owner);

                                    foreach (var member in looter.Owner.Group.GetAllCharacters())
                                    {
                                        member.Map.AddMessage(() => GlobalHandler.SendRemoveItemResponse(member.Client, lootItem.Position.X, lootItem.Position.Y));
                                        member.Map.ClearLootSlot(lootItem.Position.X, lootItem.Position.Y);
                                    }
                    ...
                    {
                        // Distribute loot to the next looter in line
                        int nextLooterIndex = (lastLooterIndex + 1) % lootItem.Loot.Looters.Count;
                        var looter = lootItem.Loot.Looters[nextLooterIndex];
                        lastLooterIndex = nextLooterIndex;
                        ...
                        if (lootItem.Template.Id == 20551)
                        {
                            looter.Owner.AddMoney((uint)(lootItem.Amount));
                            Asda2InventoryHandler.SendGoldPickupedResponse(looter.Owner.Money, looter.Owner);
                        }
                        else
                        {
                            Asda2Item asda2Item = null;
                            Asda2InventoryError asda2InventoryError = looter.Owner.Asda2Inventory.TryAdd((int)lootItem.Template.ItemId, lootItem.Amount,
                                true, ref asda2Item, new Asda2InventoryType?(), null);
                            if (asda2Item != null)
                                WCell.RealmServer.Logs.Log
                                    .Create(WCell.RealmServer.Logs.Log.Types.ItemOperations, LogSourceType.Character,
                ...
                else
                {
                    if(IsMoneyLootItem(lootItem))
                    {
                        if(ShouldShareMasterLootMoney(lootItem))
                            PickUpMoneyLootForGroup(lootItem);
                        else
                            PickUpMoneyLoot(lootItem);
                        return;
                    }
                    ...
                    if(lootItem.Template.Id == 20551)
                    {
                        Owner.AddMoney((uint)lootItem.Amount);
                        Asda2InventoryHandler.SendGoldPickupedResponse(Owner.Money, Owner);
                    }
                    else
                    {
                        Asda2Item asda2Item = null;
                        ...
                        else
                        {
                            ...
                            Owner.Map.AddMessage(() => GlobalHandler.SendRemoveItemAllResponse(Owner.Client, lootItem.Position.X, lootItem.Position.Y));
                            Owner.Map.ClearLootSlot(lootItem.Position.X, lootItem.Position.Y);
                            if (!lootItem.Loot.IsAllItemsTaken)
                                return;
                            lootItem.Loot.Dispose();
                        }
        ...
        private static bool IsMoneyLootItem(Asda2LootItem lootItem)
        {
            return lootItem != null && lootItem.Template != null &&
                   ((int)lootItem.Template.ItemId == 20551 || lootItem.Template.Id == 20551);
        }

        private bool CanPickUpProtectedLoot(Asda2LootItem lootItem)
        {
            if(Owner == null || lootItem == null || lootItem.Loot == null)
                return false;

            Asda2Loot loot = lootItem.Loot;
            if(loot.Looters == null || loot.Looters.Count == 0 ||
               loot.SpawnTime.AddSeconds(CharacterFormulas.ForeignLootPickupTimeout) <= DateTime.Now ||
               Owner.GodMode)
                return true;

            bool isOriginalLooter = loot.Looters.Any(looter => looter.Owner == Owner);
            if(!isOriginalLooter)
                return false;

            if(loot.Group != null && Owner.Group != loot.Group)
            {
                if(IsRegroupedWithProtectedOwner(loot))
                    return true;
                return loot.ProtectedOwner == null || loot.ProtectedOwner == Owner;
            }

            return true;
        }

        private bool IsRegroupedWithProtectedOwner(Asda2Loot loot)
        {
            return loot != null && loot.ProtectedOwner != null &&
                   Owner != null && Owner.Group != null &&
                   loot.ProtectedOwner.Group == Owner.Group;
        }

        private bool ShouldShareMasterLootMoney(Asda2LootItem lootItem)
        {
            return Owner != null && Owner.IsInGroup && Owner.Group != null &&
                   Owner.Group.LootMethod == Constants.Looting.LootMethod.MasterLoot &&
                   lootItem != null && lootItem.Loot != null &&
                   lootItem.Loot.Group == Owner.Group;
        }

        private List<Character> GetMasterLootMoneyReceivers(Asda2LootItem lootItem)
        {
            if(lootItem == null || lootItem.Loot == null || lootItem.Loot.Looters == null)
                return new List<Character>();

            List<Character> receivers = lootItem.Loot.Looters
                .Where(looter => looter.Owner != null && looter.Owner.Group == Owner.Group)
                .Select(looter => looter.Owner)
                .Distinct()
                .ToList();
            ...
            return Owner.Group.GetAllCharacters().ToList();
        }

        private static uint GetMoneyAmount(Asda2LootItem lootItem)
        {
            return lootItem == null || lootItem.Amount <= 0 ? 0U : (uint)lootItem.Amount;
        }

        private static uint GetGroupMoneyShare(Asda2LootItem lootItem)
        {
            if(lootItem == null || lootItem.Loot == null || lootItem.Loot.Looters == null ||
               lootItem.Loot.Looters.Count == 0)
                return 0U;

            int share = lootItem.Amount / lootItem.Loot.Looters.Count;
            return (uint)(share < 1 ? 1 : share);
        }

        private void PickUpMoneyLoot(Asda2LootItem lootItem)
        {
            uint amount = GetMoneyAmount(lootItem);
            if(amount == 0)
            {
            ...
            GiveLootMoney(Owner, amount);
            CompleteLootPickup(lootItem);
        }

        private void PickUpMoneyLootForGroup(Asda2LootItem lootItem)
        {
            uint amount = GetMoneyAmount(lootItem);
            if(amount == 0)
            {
            ...
            List<Character> members = GetMasterLootMoneyReceivers(lootItem);
            if(members.Count == 0)
            {
                GiveLootMoney(Owner, amount);
                CompleteLootPickup(lootItem);
            ...
            uint share = amount / (uint)members.Count;
            if(share < 1U)
                share = 1U;

            foreach(Character member in members)
                GiveLootMoney(member, share);

            CompleteLootPickup(lootItem);
        }

        private void GiveLootMoney(Character receiver, uint amount)
        {
        ...
        private void CompleteLootPickup(Asda2LootItem lootItem)
        {
            if(lootItem == null)
                return;

            lootItem.Taken = true;
            GlobalHandler.SendRemoveItemResponse(Owner.Client, lootItem.Position.X, lootItem.Position.Y,
              (int)lootItem.Template.ItemId);

            WCell.RealmServer.Global.Map map = Owner.Map;
            Asda2Loot loot = lootItem.Loot;
            if(map == null)
                return;
            ...
            map.AddMessage(() =>
            {
                if(loot != null && loot.Map != null)
                    GlobalHandler.SendRemoveItemResponse(lootItem);
                map.ClearLootSlot(lootItem.Position.X, lootItem.Position.Y);
                if(loot != null && loot.IsAllItemsTaken)
                    loot.Dispose();
            });
        }
            ...
            else
            {
                ...
                else
                {
                    ...
                    ItemUpgradeResult itemUpgradeResult = CharacterFormulas.CalculateItemUpgradeResult(
                      ...
                      shopItem.RequiredLevel, Owner.Asda2Luck, 0, 0, useProtect, useChanceBoost, noEnchantLose, Owner.GodMode);
        ...
        public void DeleteRecipe(short slot)
        {
            if (slot < 1 || slot >= RegularItems.Length)
            {
                Asda2CraftingHandler.SendRecipeLeadnedResponse(Owner.Client, false, 0, null);
        ...
        public Asda2Item GetRegularItemById(int id)
        {
            return RegularItems.Where((p) => p.Template.Id == id).First();
        }
        ...
        public Asda2Item GetShopItemById(int id)
        {
            return ShopItems.Where((p) => p.Template.Id == id).First();
        }
        ...
        public void FillOnCharacterCreate()
        {
            ...
            SetRegularInventoty(Asda2Item.CreateItem(20565, Owner, 500), 5, true);
        ...
        public void PushItemsToWh(IEnumerable<Asda2WhItemStub> itemStubs)
        {
            ...
            else
            {
                ...
                foreach (Asda2WhItemStub itemStub in itemStubs)
                {
                    ...
                    if (num != (int)Asda2InventoryError.Ok || asda2Item == null)
                    {
                        Asda2InventoryHandler.SendItemsPushedToWarehouseResponse(Owner.Client,
                          PushItemToWhStatus.ItemNotFounded, null, null);
                        return;
                    }

                    itemToCopyStats.Amount -= itemStub.Amount;
                    if(!itemToCopyStats.IsDeleted)
                      itemToCopyStats.Save();
                    asda2Item.Save();
