...
namespace WCell.RealmServer.NPCs.Spawns
{
    ...
    INPCDataHolder, IDataHolder
  {
    ...
    public override NPC SpawnObject(NPCSpawnPoint point)
    {
      ...
      point.Map.AddObject(npc, Position);
      return npc;
    }
    ...
    public LinkedList<WaypointEntry> CreateWaypointsForSpawn()
    {
      AIMotionGenerationType moveType = MoveType;
      if(moveType == AIMotionGenerationType.IdleMotion && Entry != null)
        moveType = Entry.MovementType;
      if(Waypoints.Count > 0)
        return CloneWaypoints();
      ITerrain terrain = TerrainMgr.GetTerrain(MapId);
      if(terrain == null)
        return WaypointEntry.EmptyList;
      if(RespawnRadius <= 0.0f)
      {
        if(moveType == AIMotionGenerationType.RandomMotion)
          return CreateWaypointList(
            new RandomWaypointGenerator().GenerateWaypoints(terrain, Position));
        return WaypointEntry.EmptyList;
      }
      return CreateWaypointList(
        new RandomWaypointGenerator().GenerateWaypoints(terrain, Position, RespawnRadius));
    ...
    private LinkedList<WaypointEntry> CloneWaypoints()
    {
      LinkedList<WaypointEntry> list = new LinkedList<WaypointEntry>();
      foreach(WaypointEntry waypoint in Waypoints)
      {
        WaypointEntry copy = new WaypointEntry
        {
          SpawnEntry = this,
          SpawnId = SpawnId,
          Id = waypoint.Id,
          ...
          Emote = waypoint.Emote,
          SpellId = waypoint.SpellId,
          ArriveDisplayId = waypoint.ArriveDisplayId,
          LeaveDisplayId = waypoint.LeaveDisplayId
        };
        copy.Node = list.AddLast(copy);
      }

      return list;
    }

    private LinkedList<WaypointEntry> CreateWaypointList(Vector3[] positions)
    {
      LinkedList<WaypointEntry> list = new LinkedList<WaypointEntry>();
      ...
      for(int index = 0; index < positions.Length; ++index)
      {
        ...
        float orientation = index < positions.Length - 1
          ? position.GetAngleTowards(positions[index + 1])
          : Utility.Random(0.0f, 6.283185f);
        WaypointEntry waypoint = new WaypointEntry
        {
          SpawnEntry = this,
          SpawnId = SpawnId,
          Id = (uint) (index + 1),
          ...
          WaitTime = index == positions.Length - 1 ? (uint) Utility.Random(2000, 7000) : 0U
        };
        waypoint.Node = list.AddLast(waypoint);
      }
