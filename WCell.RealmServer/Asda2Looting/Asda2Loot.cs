...
namespace WCell.RealmServer.Looting
{
    ...
    public class Asda2Loot : WorldObject
    {
        ...
        public Character ProtectedOwner { get; set; }

        protected Asda2Loot(IAsda2Lootable looted, uint money, Asda2LootItem[] items, byte channel)
            : this()
        {
            this.Money = money;
            this.Items = items;
        ...
        public void Initialize(Character chr, IList<Asda2LooterEntry> looters, MapId mapid)
        {
            ...
            this.Method = LootMethod.NeedBeforeGreed;
        }
        ...
        public bool GiveItems()
        {
            if (!(this.Lootable is NPC))
                return false;
            if (this.Group == null || this.Method == LootMethod.MasterLoot)
            {
                Asda2LooterEntry asda2LooterEntry = this.Looters.FirstOrDefault<Asda2LooterEntry>();
                if (asda2LooterEntry != null && asda2LooterEntry.Owner != null)
                {
                    ...
                    {
                        if(asda2LootItem.Template.Id == 20551)
                        {
                            if(this.Group != null)
                                SendMoneyToGroup((uint)asda2LootItem.Amount);
                            else
                                SendMoney(asda2LooterEntry.Owner, (uint)asda2LootItem.Amount);
                        }
                        else
                        {
            ...
            else
            {
                ...
                if (characterList.Count > 0)
                {
                    Asda2LootItem[] asda2LootItemListList = this.Items;
                    int index1 = 0;
                    //if (characterList.Count == 1)
                    //{
                    //    this.Items.ForEach(item => asda2LootItemListList.Add(item));
                    //    asda2LootItemListList.Add();
                    //}
                    //else
                    //{
                    ...
                    //    }
                    //}
                    foreach (Asda2LootItem asda2LootItem in asda2LootItemListList)
                    {
                        Random random = new Random();
                        if (asda2LootItem.Amount > 1)
                        {
                            foreach (Character chr in characterList)
                            {
                                int div = random.Next(1, asda2LootItem.Amount);
                                if (asda2LootItem.Template.Id == 20551)
                                {
                                    SendMoney(chr, (uint)asda2LootItem.Amount);
                                }
                                else
                                {
                                    Asda2Item asda2Item = (Asda2Item)null;
                                    Asda2InventoryError asda2InventoryError =
                                        chr.Asda2Inventory.TryAdd((int)asda2LootItem.Template.ItemId, asda2LootItem.Loot.Method == LootMethod.GroupLoot ? asda2LootItem.Amount / characterList.Count : asda2LootItem.Amount / div,
                        ...
                        {
                            int index = random.Next(1, Looters.Count);
                            Character chr = this.Looters[index].Owner;
                            if (chr != null)
                            {
                                if (asda2LootItem.Template.Id == 20551)
                                {
        ...
        private void SendMoneyToGroup(uint amount)
        {
            if(this.Group == null)
                return;

            Character[] members = this.Group.GetAllCharacters();
            if(members.Length == 0)
                return;

            uint share = amount / (uint)members.Length;
            if(share < 1U)
                share = 1U;

            foreach(Character member in members)
                SendMoney(member, share);
        }
