...
namespace WCell.RealmServer.Asda2Titles
{
  public static class Asda2TitlesHandler
  {
    ...
    public static void SetTitleRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      if(num1 != (short) -1)
      {
        if(!IsValidRequestedTitleId(num1))
        {
      ...
      if(num2 != (short) -1)
      {
        if(!IsValidRequestedTitleId(num2))
        {
          client.ActiveCharacter.YouAreFuckingCheater("Wrong title id : " + (object) num2, 1);
          return;
        }
    ...
    private static bool IsValidRequestedTitleId(short titleId)
    {
      return titleId >= -1 && titleId < Asda2TitleTemplate.MaxTitleCount;
    }
  }
}
