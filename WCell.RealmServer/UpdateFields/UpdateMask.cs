...
namespace WCell.RealmServer.UpdateFields
{
  public class UpdateMask
  {
    ...
    {
      m_blocks = data ?? new uint[0];
      m_maxBlockCount = m_blocks.Length;
      m_highestIndex = 0;
      m_lowestIndex = int.MaxValue;
      DataByOne = new byte[m_blocks.Length * 4 * 8];
    ...
    public void UnsetBit(int index)
    {
      ...
      if(DataByOne != null && index < DataByOne.Length)
        DataByOne[index] = 0;
    }
    ...
    private void EnsureBlockCount(int index)
    {
      if(index < 0)
        return;
      int blockCount = (index >> 5) + 1;
      if(blockCount <= m_blocks.Length)
        return;
      Array.Resize(ref m_blocks, blockCount);
      m_maxBlockCount = blockCount;
      if(DataByOne == null)
        return;
      byte[] dataByOne = DataByOne;
      Array.Resize(ref dataByOne, blockCount * 4 * 8);
      DataByOne = dataByOne;
    }
  }
}
