...
namespace WCell.RealmServer.Modifiers
{
  ...
  public static class UnitUpdates
  {
    ...
    {
      IAsda2Weapon mainWeapon = unit.MainWeapon;
      int attackTime = mainWeapon.AttackTime;
      int num = GetMultiMod(
        unit.FloatMods[1] -
        CharacterFormulas.CalculateAtackTimeReduce(unit.Level, unit.Class, unit.Asda2Agility), attackTime);
      Asda2Item item = mainWeapon as Asda2Item;
      if(item != null && item.Category == Asda2ItemCategory.Ballista)
      {
        int speedupPct = Asda2CombatHandler.BallistaAttackSpeedupPct;
        if(speedupPct < 0)
          speedupPct = 0;
        if(speedupPct > 50)
          speedupPct = 50;
        num = num * (100 - speedupPct) / 100;
      }
      if(num < 30)
        num = 30;
