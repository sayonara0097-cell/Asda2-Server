...
namespace WCell.RealmServer.Entities
{
      ...
      IWorldLocation, IHasPosition
    {
        ...
        public static int ClientAttackSyncMinElapsedPct = 45;
        public static int ClientAttackSyncMinIntervalMillis = 300;
        public static int ClientAttackSyncDuplicateGuardMillis = 100;
        ...
        private const int Asda2CityEntryHealthRegenDurationMillis = 8000;
        private const int Asda2SittingCityHealthRegenDurationMillis = 10000;
        private const int Asda2SittingFieldHealthRegenDurationMillis = 15000;
        private const int Asda2FastHealthRegenTickMillis = 1000;
        ...
        protected internal DateTime m_lastMoveTimeT;
        ...
        private int _asda2FastHealthRegenToken;
        private bool _asda2FastHealthRegenActive;
        private bool _asda2FastHealthRegenRequiresSitting;
        private bool _asda2FastHealthRegenRequiresCity;
        ...
        protected int m_clientAttackSyncId;
        protected bool m_clientAttackPulseProcessedSincePrepare;
        protected int m_clientAttackInstantStrikesRemaining;
        protected bool m_skipNextClientAttackPulse;
        protected int m_skipNextClientAttackPulseUntil;
        protected int m_clientAttackAnimationToken;
        ...
        public virtual int Health
        {
            ...
            set
            {
                ...
                if (value < 1)
                {
                    ...
                    {
                        Asda2Pvp duel = character.Asda2Duel;
                        SetUInt32(UnitFields.HEALTH, (uint)Math.Max(1, MaxHealth / 2));
                        if (duel != null && !duel.IsFinishScheduled)
                            duel.FinishAfterPreventedDeath(character);
                    }
                    else
                        Die(false);
                }
        ...
        public bool IsSitting
        {
            ...
            set
            {
                ...
                if (value)
                    StartAsda2SittingHealthRegen();
                else
                    StopAsda2SittingHealthRegen();
            }
        }
        ...
        public bool IsGhost
        {
            get { return m_auras != null && m_auras.GhostAura != null; }
        }
        ...
        private bool IsInAsda2CityMap()
        {
            if (!(this is Character) || Map == null)
                return false;
            return Map.Id == MapId.Alpia || Map.Id == MapId.Silaris ||
                   Map.Id == MapId.Flamio || Map.Id == MapId.Aquaton;
        }

        protected void FillAsda2HealthOnWorldEnter()
        {
            if (MaxHealth <= 0 || Health >= MaxHealth)
                return;
            CancelAsda2FastHealthRegen();
            Health = MaxHealth;
        }

        protected void StartAsda2CityEntryHealthRegen()
        {
            if (!IsInAsda2CityMap())
                return;
            StartAsda2TimedHealthRegen(Asda2CityEntryHealthRegenDurationMillis, false, true);
        }

        private void StartAsda2SittingHealthRegen()
        {
            int duration = IsInAsda2CityMap()
              ? Asda2SittingCityHealthRegenDurationMillis
              : Asda2SittingFieldHealthRegenDurationMillis;
            StartAsda2TimedHealthRegen(duration, true, false);
        }

        private void StartAsda2TimedHealthRegen(int durationMillis, bool requiresSitting, bool requiresCity)
        {
            Character chr = this as Character;
            Map map = Map;
            if (chr == null || map == null || !IsAlive || MaxHealth <= 0 || Health >= MaxHealth)
            {
                _asda2FastHealthRegenActive = false;
                return;
            }
            if (requiresCity && !IsInAsda2CityMap())
                return;
            if (requiresSitting && !IsSitting)
                return;
            if (_asda2FastHealthRegenActive &&
                _asda2FastHealthRegenRequiresSitting == requiresSitting &&
                _asda2FastHealthRegenRequiresCity == requiresCity)
                return;

            _asda2FastHealthRegenActive = true;
            _asda2FastHealthRegenRequiresSitting = requiresSitting;
            _asda2FastHealthRegenRequiresCity = requiresCity;
            int token = ++_asda2FastHealthRegenToken;
            int tickCount = Math.Max(1, durationMillis / Asda2FastHealthRegenTickMillis);
            ScheduleAsda2FastHealthRegenTick(chr, map, token, tickCount, 1);
        }

        private void ScheduleAsda2FastHealthRegenTick(Character chr, Map map, int token, int tickCount, int tick)
        {
            map.CallDelayed(Asda2FastHealthRegenTickMillis,
              () => ApplyAsda2FastHealthRegenTick(chr, map, token, tickCount, tick));
        }

        private void StopAsda2FastHealthRegen(int token)
        {
            if (token != _asda2FastHealthRegenToken)
                return;
            _asda2FastHealthRegenActive = false;
            _asda2FastHealthRegenRequiresSitting = false;
            _asda2FastHealthRegenRequiresCity = false;
        }

        private void StopAsda2SittingHealthRegen()
        {
            if (!_asda2FastHealthRegenActive || !_asda2FastHealthRegenRequiresSitting)
                return;
            CancelAsda2FastHealthRegen();
        }

        private void CancelAsda2FastHealthRegen()
        {
            ++_asda2FastHealthRegenToken;
            _asda2FastHealthRegenActive = false;
            _asda2FastHealthRegenRequiresSitting = false;
            _asda2FastHealthRegenRequiresCity = false;
        }

        private void ApplyAsda2FastHealthRegenTick(Character chr, Map map, int token, int tickCount, int tick)
        {
            if (token != _asda2FastHealthRegenToken)
                return;
            if (!_asda2FastHealthRegenActive || chr == null || chr.IsDeleted || chr.Map != map ||
                chr.Client == null || !chr.Client.IsGameServerConnection || !IsAlive)
            {
                StopAsda2FastHealthRegen(token);
                return;
            }
            if (_asda2FastHealthRegenRequiresCity && !IsInAsda2CityMap())
            {
                StopAsda2FastHealthRegen(token);
                return;
            }
            if (_asda2FastHealthRegenRequiresSitting && !IsSitting)
            {
                StopAsda2FastHealthRegen(token);
                return;
            }
            ...
            if (Health < MaxHealth)
            {
                int stepsLeft = Math.Max(1, tickCount - tick + 1);
                int missingHealth = MaxHealth - Health;
                int healAmount = Math.Max(1, (int)Math.Ceiling(missingHealth / (float)stepsLeft));
                Health = Math.Min(MaxHealth, Health + healAmount);
                Asda2CharacterHandler.SendHealthUpdate(chr, false, false);
            }

            if (Health >= MaxHealth || tick >= tickCount)
            {
                StopAsda2FastHealthRegen(token);
                return;
            }

            ScheduleAsda2FastHealthRegenTick(chr, map, token, tickCount, tick + 1);
        }
        ...
        protected void Regenerate(int dt)
        {
            if (!IsRegenerating)
                return;
            Character characterMaster = this.CharacterMaster;
            if (characterMaster == null || characterMaster.Asda2Inventory == null)
                return;
            if (characterMaster.Asda2Inventory.WeightPrc >= 75)
                return;
            int health = Health;
            _tempHealthRegen +=
              (m_isInCombat ? HealthRegenPerTickCombat : HealthRegenPerTickNoCombat) *
            ...
            if (_timeFromLastHealthUpdate >= 5000)
            {
                ...
                if (_tempHealthRegen > 1.0)
                {
                    ...
                    if (Health < MaxHealth)
                    {
                        Health = Math.Min(MaxHealth, health + tempHealthRegen);
                        Character chr = this as Character;
                        if (chr != null)
                            Asda2CharacterHandler.SendHealthUpdate(chr, false, false);
                    }
        ...
        /// <param name="healer">The object that heals this Unit (or null)</param>
        /// <param name="effect">The effect of the spell that triggered the healing (or null)</param>
        public void Heal(int value, Unit healer = null, SpellEffect effect = null, bool animate = true)
        {
            Character chr = this as Character;
            bool flag = false;
            int num1 = 0;
            ...
            if (value > 0)
            {
                ...
                value += num1;
                if (chr != null)
                    Asda2CharacterHandler.SendHealthUpdate(chr, animate, false);
            }
        ...
        {
            if (ShouldSuppressAsda2PostDuelPeriodicDamage(attacker, effect))
                return;
            int power = Power;
            amount -= MathUtil.RoundInt((float)(amount * (double)GetResiliencePct() * 2.20000004768372));
            if (amount > power)
        ...
        private bool ShouldSuppressAsda2PostDuelPeriodicDamage(Unit attacker, SpellEffect effect)
        {
            if (attacker == null || effect == null || !effect.IsPeriodic)
                return false;
            Character attackerCharacter = attacker.CharacterMaster;
            Character victimCharacter = CharacterMaster;
            if (attackerCharacter == null || victimCharacter == null || attackerCharacter == victimCharacter)
                return false;
            return !attackerCharacter.CanHarm(victimCharacter);
        }
        ...
        {
            if (obj == null || !IsInWorld || m_auras == null)
                return false;
            ...
            Character viewerCharacter = this as Character;
            if (viewerCharacter != null && viewerCharacter.Client == null)
                return false;
            if (this == obj)
            {
                return true;
            ...
            if (obj is Character)
            {
                ...
                if (!character.IsInWorld || character.Client == null)
                    return false;
                ...
                if (this is NPC npc)
                {
                    return character.Client.Channel == npc.Channel;
                }
            }
        ...
        {
            if (action != null && HasLastResortProtectionAgainst(action.Attacker))
            {
                action.Damage = 0;
                action.Absorbed = 0;
                action.Resisted = 0;
                ...
                action.VictimState = VictimState.Miss;
                action.HitFlags = HitFlags.Miss;
                action.SetActualDamageOverride(0);
                return;
            }
        ...
        internal bool HasLastResortProtectionAgainst(Unit attacker)
        {
            return attacker != null &&
                   attacker != this &&
                   Auras != null &&
                   Auras.Contains(SpellLineId.LastResort);
        }
        ...
        public void Strike(IAsda2Weapon weapon)
        {
            ...
            Character targetChr = target as Character;
            Character attackerChr = unusedAction.Attacker as Character;
            Strike(weapon, unusedAction, target);

            if (targetChr != null && attackerChr != null)
            {
                if (targetChr.Damages.ContainsKey(attackerChr))
                {
                    var dmg = targetChr.Damages[attackerChr];
                    targetChr.Damages[attackerChr] = dmg + unusedAction.ActualDamage;
                }
                else
                {
                    targetChr.Damages[attackerChr] = unusedAction.ActualDamage;
                }
            }
        }
        ...
        public ProcHitFlags Strike(IAsda2Weapon weapon, DamageAction action, Unit target, SpellCast ability)
        {
            ...
            action.ClearActualDamageOverride();
          ...
          bool mayCrit = true, bool forceCrit = false, bool clearAction = true)
        {
            ...
            if (IsEvading || IsImmune(school) || (IsInvulnerable || !IsAlive))
                return null;
            DamageAction unusedAction = GetUnusedAction();
            unusedAction.ClearActualDamageOverride();
            ...
            set
            {
                ...
                if (m_isFighting = value)
                {
                    ...
                    else
                    {
                        ...
                        if (this is NPC)
                            IsInCombat = true;
                        else if (this is Character && (m_target is NPC || m_target is Character))
                            m_attackTimer.Stop();
                        else
                            m_attackTimer.Start(1);
                    }
        ...
        public void PrepareClientAttackSync()
        {
            int attackDelay = GetClientAttackSyncDelay();
            m_lastStrike = Environment.TickCount - attackDelay;
            ++m_clientAttackSyncId;
            m_clientAttackPulseProcessedSincePrepare = false;
            m_clientAttackInstantStrikesRemaining = 0;
            m_skipNextClientAttackPulse = false;
            m_skipNextClientAttackPulseUntil = 0;
            m_clientAttackAnimationToken = 1;
            m_attackTimer.Stop();
        }

        public int ClientAttackSyncId
        {
            get { return m_clientAttackSyncId; }
        }

        public int ClientAttackAnimationToken
        {
            get { return m_clientAttackAnimationToken; }
        }

        public bool HasClientAttackInstantStrikesRemaining
        {
            get { return m_clientAttackInstantStrikesRemaining > 0; }
        }

        public void SetClientAttackInstantStrikesRemaining(int count)
        {
            m_clientAttackInstantStrikesRemaining = Math.Max(0, count);
        }

        public void ConsumeClientAttackInstantStrike()
        {
            if (m_clientAttackInstantStrikesRemaining > 0)
                --m_clientAttackInstantStrikesRemaining;
        }

        public void SetClientAttackAnimationToken(int token)
        {
            if (token <= 0)
                return;
            m_clientAttackAnimationToken = token;
        }

        public void AdvanceClientAttackAnimationToken()
        {
            ++m_clientAttackAnimationToken;
            if (m_clientAttackAnimationToken <= 0 || m_clientAttackAnimationToken > 127)
                m_clientAttackAnimationToken = 1;
        }

        public bool IsProcessingInitialClientAttackPulse
        {
            get { return m_clientAttackSyncId > 0 && !m_clientAttackPulseProcessedSincePrepare; }
        }

        public int GetClientAttackSyncDelay()
        {
            int attackDelay = IsUsingRangedWeapon ? RangedAttackTime : MainHandAttackTime;
            return attackDelay > 0 ? attackDelay : 1000;
        }

        public bool ProcessClientAttackPulse()
        {
            return ProcessClientAttackPulse(null);
        }

        public bool ProcessClientAttackPulse(Action beforeStrike)
        {
            if (!m_isFighting)
                IsFighting = true;

            if (!m_isFighting || m_target == null)
                return false;

            int now = Environment.TickCount;
            int attackDelay = GetClientAttackSyncDelay();
            int elapsed = now - m_lastStrike;

            if (this is Character && (m_target is NPC || m_target is Character))
            {
                if (ShouldSkipNextClientAttackPulse(now))
                    return false;

                int duplicateGuard = Math.Max(1, ClientAttackSyncDuplicateGuardMillis);
                if (m_lastStrike != 0 && elapsed >= 0 && elapsed < duplicateGuard)
                    return false;

                m_attackTimer.Stop();
                if (elapsed < attackDelay)
                    m_lastStrike = now - attackDelay;

                if (beforeStrike != null)
                    beforeStrike();
                CombatTick(0);
                m_attackTimer.Stop();
                m_clientAttackPulseProcessedSincePrepare = true;
                return true;
            }

            int requiredElapsed = Math.Min(attackDelay, Math.Max(ClientAttackSyncMinIntervalMillis,
                attackDelay * ClientAttackSyncMinElapsedPct / 100));
            m_attackTimer.Stop();

            if (m_lastStrike == 0 || elapsed >= requiredElapsed)
            {
                if (elapsed < attackDelay)
                    m_lastStrike = now - attackDelay;
            ...
            {
                m_attackTimer.Start(requiredElapsed - elapsed);
                return true;
            }
        ...
        public bool ProcessClientAttackStrike(Unit expectedTarget, IAsda2Weapon weapon)
        {
            return ProcessClientAttackStrike(expectedTarget, weapon, null);
        }

        public bool ProcessClientAttackStrike(Unit expectedTarget, IAsda2Weapon weapon,
            Action beforeStrike)
        {
            return ProcessClientAttackStrike(expectedTarget, weapon, beforeStrike, false);
        }

        public bool ProcessClientAttackInstantStrike(Unit expectedTarget, IAsda2Weapon weapon,
            Action beforeStrike)
        {
            if (m_clientAttackInstantStrikesRemaining <= 0)
                return ProcessClientAttackStrike(expectedTarget, weapon, beforeStrike, false);

            bool processed = ProcessClientAttackStrike(expectedTarget, weapon, beforeStrike, true);
            if (processed)
                ConsumeClientAttackInstantStrike();
            return processed;
        }

        private bool ProcessClientAttackStrike(Unit expectedTarget, IAsda2Weapon weapon,
            Action beforeStrike, bool ignoreClientPulseTiming)
        {
            if (!m_isFighting)
                IsFighting = true;

            if (!m_isFighting || m_target == null || expectedTarget == null ||
                m_target != expectedTarget || weapon == null)
                return false;

            RefreshCurrentPosition();
            expectedTarget.RefreshCurrentPosition();

            int now = Environment.TickCount;
            if (!ignoreClientPulseTiming && ShouldSkipNextClientAttackPulse(now))
                return false;

            int duplicateGuard = Math.Max(1, ClientAttackSyncDuplicateGuardMillis);
            int elapsed = now - m_lastStrike;
            if (!ignoreClientPulseTiming && m_lastStrike != 0 && elapsed >= 0 &&
                elapsed < duplicateGuard)
                return false;

            if (!IsInAttackRangeSq(weapon, expectedTarget, GetDistanceSq(expectedTarget)))
            {
                Character character = this as Character;
                if (character != null)
                    CombatHandler.SendAttackSwingNotInRange(character);
                return false;
            }

            if (ignoreClientPulseTiming)
            {
                m_skipNextClientAttackPulse = false;
                m_skipNextClientAttackPulseUntil = 0;
            }
        ...
        private bool ShouldSkipNextClientAttackPulse(int now)
        {
            if (!m_skipNextClientAttackPulse)
                return false;

            if (m_skipNextClientAttackPulseUntil == 0 ||
                unchecked(m_skipNextClientAttackPulseUntil - now) > 0)
            {
                m_skipNextClientAttackPulse = false;
                m_skipNextClientAttackPulseUntil = 0;
                return true;
        ...
        public bool TryProcessScheduledClientAttackPulse(int syncId, Unit expectedTarget)
        {
            if (syncId != m_clientAttackSyncId || m_clientAttackPulseProcessedSincePrepare ||
                expectedTarget == null || m_target != expectedTarget)
                return false;

            return ProcessClientAttackPulse();
        }

        public bool TryProcessScheduledClientAttackStrike(int syncId, Unit expectedTarget,
            IAsda2Weapon weapon)
        {
            if (syncId != m_clientAttackSyncId || m_clientAttackPulseProcessedSincePrepare ||
                expectedTarget == null || m_target != expectedTarget)
            ...
            return ProcessClientAttackStrike(expectedTarget, weapon);
        }

        public void SkipNextClientAttackPulseForMillis(int delay)
        {
            m_skipNextClientAttackPulse = true;
            m_skipNextClientAttackPulseUntil = Environment.TickCount + Math.Max(1, delay);
        }
        ...
        public void RefreshCurrentPosition()
        {
            ...
            if (character != null)
            {
                if (character.IsMoving)
                    Asda2MovmentHandler.CalculateAndSetRealPos(character, 0);
                return;
            }

            if (IsMoving)
                Movement.UpdatePositionNow();
        }
        ...
        public void DoRawDamage(IDamageAction action)
        {
            ...
            Character asda2DuelLoser = PreventAsda2DuelDeath(action, ref actualDamage);
            if (actualDamage > 0)
                action.Victim.OnDamageAction(action);
            if (actualDamage <= 0)
            {
                FinishAsda2DuelAfterPreventedDeath(asda2DuelLoser);
                return;
            }
            if (m_brain != null)
        ...
        private Character PreventAsda2DuelDeath(IDamageAction action, ref int actualDamage)
        {
            Character victim = this as Character;
            if (victim == null || !victim.IsAsda2Dueling || victim.Asda2Duel == null ||
                action.Attacker == null)
                return null;

            Character attacker = action.Attacker.CharacterMaster;
            if (attacker == null || attacker != victim.Asda2DuelingOponent)
                return null;
            if (victim.Asda2Duel.IsFinishScheduled)
            {
                actualDamage = 0;
                return null;
            }
            ...
            int targetHealth = Math.Max(1, victim.MaxHealth / 2);
            int allowedDamage = Math.Max(0, victim.Health - targetHealth);
            actualDamage = allowedDamage;
            return victim;
        }

        private static void FinishAsda2DuelAfterPreventedDeath(Character loser)
        {
            if (loser == null || !loser.IsAsda2Dueling || loser.Asda2Duel == null)
                return;
            Asda2Pvp duel = loser.Asda2Duel;
            duel.FinishAfterPreventedDeath(loser);
        }
