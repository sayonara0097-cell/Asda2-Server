...
using static Castle.ActiveRecord.Framework.DictionaryAdapter;
...
namespace WCell.RealmServer.Entities
{
      ...
      IEntity, ITicketHandler, IGenericChatTarget, INamed, IHasRole, IInstanceHolderSet, ICharacterSet, IPacketReceiver
    {
        ...
        public new static readonly List<Character> EmptyArray = new List<Character>();
        private const double NpcVisibilityRefreshDelayMillis = 900.0;
        ...
        private DateTime _LastMoveStarted;
        private DateTime _LastMoveEnded;
        ...
        public Vector3 LastPositionBeforeMove;
        ...
        public override void OnMove()
        {
            ...
            if (CurrentCapturingPoint != null)
                CurrentCapturingPoint.StopCaptureIfMovedAway(this);
        ...
        public void OnOutOfRange(WorldObject obj)
        {
            ...
            else
            {
                ...
                if (CanSendAsda2NpcVisible(npc))
                {
                    GlobalHandler.SendNpcDeleteResponse(Client, npc);
                    return;
                }
        ...
        {
            base.OnEnterMap();
            StartAsda2CityEntryHealthRegen();
            if (!_saveTaskRunning)
            {
        ...
        public DateTime LastMoveStarted
        {
            get { return _LastMoveStarted; }
            set { _LastMoveStarted = value; }
        }

        public DateTime LastMoveEnded
        {
            get { return _LastMoveEnded; }
            set { _LastMoveEnded = value; }
        }
        ...
        {
            get { return (LastMoveEnded - LastMoveStarted).TotalSeconds; }
        }
        ...
        public int TransportItemId
        {
            ...
            set
            {
                ...
                if (value != -1)
                    GlobalHandler.SendBuffsOnCharacterInfoResponse(Client, this);
            }
        }
        ...
        public int MountId
        {
            ...
            set
            {
                ...
                else if (Asda2MountHandler.GetMountCooldownRemaining(this) > TimeSpan.Zero)
                {
        ...
        public bool CheckQuest(int templateId)
        {
            return Asda2QuestProgressRecord.GetActiveQuestRecordsForCharacter(EntityId.Low)
                .Any(q => q != null && q.QuestTemplateId == templateId);
        }
        ...
        public bool HasPendingChargeAnimationPath { get; set; }

        public Vector3 PendingChargeAnimationStart { get; set; }

        public Vector3 PendingChargeAnimationEnd { get; set; }
        ...
        public UpdateMask LearnedTutorials { get; set; }
        ...
        public void ForfeitAsda2Duel()
        {
            Asda2Pvp duel = Asda2Duel;
            if (duel != null)
            {
                duel.Losser = this;
                return;
            ...
            Character opponent = Asda2DuelingOponent;
            if (opponent != null && opponent.Asda2DuelingOponent == this)
                opponent.Asda2DuelingOponent = null;
            Asda2DuelingOponent = null;
        }


        public DateTime LastCharge = DateTime.MinValue;
        public DateTime LastPing = DateTime.MinValue;
        public DateTime LastNpcVisibilityRefresh = DateTime.MinValue;
        public DateTime LastStoreCall = DateTime.MinValue;

        private byte _GreenCharges = 0;
        private byte _BlueCharges = 0;
        private byte _RedCharges = 0;
        private bool _SoulBuffed1 = false;
        private bool _SoulBuffed2 = false;
        private bool _SoulBuffed3 = false;
        private int _CurrentSoulGuardId = int.MinValue;

        public bool SoulBuffed1
        {
            get { return _SoulBuffed1; }
            set
            {
                _SoulBuffed1 = value;
            }
        ...
        public bool SoulBuffed2
        {
            get { return _SoulBuffed2; }
            set
            {
                _SoulBuffed2 = value;
            }
        ...
        public bool SoulBuffed3
        {
            get { return _SoulBuffed3; }
            set
            {
                _SoulBuffed3 = value;
            }
        }
        public int CurrentSoulGuardId
        {
            get { return _CurrentSoulGuardId; }
            set { _CurrentSoulGuardId = value; }
        }
        public byte GreenCharges
        {
            get { return _GreenCharges; }
            set
            {
        ...
        public byte BlueCharges
        {
            get { return _BlueCharges; }
            set
            {
                LastCharge = DateTime.Now;
                _BlueCharges = value;
        ...
        public byte RedCharges
        {
            get { return _RedCharges; }
            set
            {
                LastCharge = DateTime.Now;
                _RedCharges = value;
        ...
        public bool IsChangingAsda2BattleGroundMap { get; set; }
        ...
        public static Asda2WarPoint GetNearestWarPoint(Character character, List<Asda2WarPoint> warPoints)
        {
            return warPoints
                .OrderBy(wp => GetDistance(character.Asda2Position.X, character.Asda2Position.Y, wp.X, wp.Y))
                .FirstOrDefault();
        }
        ...
        public Asda2WarPoint Asda2WarPoint;
        ...
        protected override void OnDeath()
        {
            ...
            else
            {
                ...
                if (IsAsda2BattlegroundInProgress && MapId == MapId.BatleField)
                {
                    List<Asda2WarPoint> points = new List<Asda2WarPoint>();
                    foreach (var point in this.CurrentBattleGround.Points)
                    {
                        if(point.OwnedFaction == this.Asda2FactionId)
                        {
                            points.Add(point);
                        }
                    }
                    ...
                    Asda2WarPoint nearestPoint = GetNearestWarPoint(this, points);

                    if (nearestPoint != null)
                    {
                        Asda2WarPoint = nearestPoint;
                        Map.CallDelayed(1000, () => Asda2CharacterHandler.SendResurrectOptionsResponse(Client, this, Asda2CharacterHandler.ResurrectOptions.NearestOccipiedCrystal));
                    }
                    else
                    {
                        Console.WriteLine("No war points available.");
                    }
        ...
        protected internal override void OnDamageAction(IDamageAction action)
        {
            ...
            if (action.Attacker == null)
                return;
            if (CurrentCapturingPoint != null && action.ActualDamage > 0 && action.Attacker != this)
        ...
        public void SendQuestMsg(string msg)
        {
            SendMessage("~Quest~", msg, Color.Gold);
        }
        ...
        public void SendMonsterMsg(string name, string msg)
        {
            SendMessage($"~{name}~", msg, Color.White);
        }
        ...
        {
            if (Profession == Asda2Profession.Warrior)
            {
                if (Archetype.ClassId == ClassId.OHS)
                {
                    return 0;
                }
                else if (Archetype.ClassId == ClassId.Spear)
                {
                    return 2;
                }
                else if (Archetype.ClassId == ClassId.THS)
                {
                    return 6;
                }
                else
            ...
            else if (Profession == Asda2Profession.Archer)
            {
                if (Archetype.ClassId == ClassId.Crossbow)
                {
                    return 9;
                }
                else if (Archetype.ClassId == ClassId.Bow)
                {
                    return 12;
                }
                else if (Archetype.ClassId == ClassId.Balista)
                {
                    return 15;
                }
                else
            ...
            else if (Profession == Asda2Profession.Mage)
            {
                if (Archetype.ClassId == ClassId.HealMage)
                {
                    return 18;
                }
                else if (Archetype.ClassId == ClassId.SupportMage)
                {
                    return 21;
                }
                else if (Archetype.ClassId == ClassId.AtackMage)
                {
                    return 24;
                }
                else
        ...
        {
            if (chr == null || !chr.IsAlive || chr.Map == null)
                return false;
            if (CanDuel(chr))
                return true;
            if (Asda2DigMgr.IsInTown(this) || Asda2DigMgr.IsInTown(chr))
                return false;
        ...
        {
            return IsAsda2Dueling && Asda2Duel.IsFightStarted && chr == Asda2DuelingOponent;
        }
        ...
        public override bool IsAlliedWith(IFactionMember opponent)
        {
            ...
            if (opponent is Character)
            {
                Character character = (Character)opponent;
                if (CurrentBattleGround != null && character.CurrentBattleGround == CurrentBattleGround &&
                    Asda2FactionId >= 0 && character.Asda2FactionId == Asda2FactionId)
                    return true;
                if (IsInBattleground)
                    return Battlegrounds.Team == character.Battlegrounds.Team;
                Group group = Group;
                if (group != null && character.Group == group && DuelOpponent == null)
                    return character.DuelOpponent == null;
            }
        ...
        public override bool IsInSameDivision(IFactionMember opponent)
        {
            ...
            if (opponent is Character)
            {
                ...
                if (subGroup != null && character.SubGroup == subGroup && DuelOpponent == null)
                    return character.DuelOpponent == null;
            }
        ...
        public int GetParameterMinDefSum()
        {
            int sum = 0;

            foreach (Asda2Item item in Asda2Inventory.Equipment.ToArray())
            {
                if (item != null)
                {
                    if (item.Parametr1Type == Asda2ItemBonusType.MinDef)
                    {
                        sum += item.Parametr1Value;
                    }
                    if (item.Parametr2Type == Asda2ItemBonusType.MinDef)
                    {
                        sum += item.Parametr2Value;
                    }
                    if (item.Parametr3Type == Asda2ItemBonusType.MinDef)
                    {
                        sum += item.Parametr3Value;
                    }
                    if (item.Parametr4Type == Asda2ItemBonusType.MinDef)
                    {
                        sum += item.Parametr4Value;
                    }
                }
            }
        ...
        public int GetParameterMaxDefSum()
        {
            int sum = 0;
            ...
            foreach (Asda2Item item in Asda2Inventory.Equipment.ToArray())
            {
                if (item != null)
                {
                    if (item.Parametr1Type == Asda2ItemBonusType.MaxDef)
                    {
                        sum += item.Parametr1Value;
                    }
                    if (item.Parametr2Type == Asda2ItemBonusType.MaxDef)
                    {
                        sum += item.Parametr2Value;
                    }
                    if (item.Parametr3Type == Asda2ItemBonusType.MaxDef)
                    {
                        sum += item.Parametr3Value;
                    }
                    if (item.Parametr4Type == Asda2ItemBonusType.MaxDef)
                    {
                        sum += item.Parametr4Value;
                    }
                }
        ...
        protected internal void InitializeCharacter()
        {
            ...
            try
            {
                ...
                Record.GetedTitles = NormalizeTitleMask(Record.GetedTitles);
                Record.DiscoveredTitles = NormalizeTitleMask(Record.DiscoveredTitles);
                ...
                LearnedTutorials = new UpdateMask(Record.LearnedTutorials);
                ...
                {
                    Asda2TitleTemplate titleTemplate = Asda2TitleTemplate.GetTemplate(index);
                    if (titleTemplate != null && GetedTitles.GetBit(index))
                        Asda2TitlePoints += titleTemplate.Points;
                }
        ...
        private void OnLogin()
        {
            ...
            {
                WorldObject existingMapObject = Map.GetObject(EntityId);
                if (existingMapObject == null)
                    Map.AddObjectNow(this);
                else if (!ReferenceEquals(existingMapObject, this))
                {
                    LogUtil.WarnException(
                      "Skipping duplicate map add for character {0} ({1}); another object with same EntityId is already on map.",
                      Name, EntityId);
                    return;
                }
                ...
                ScheduleWorldEnterVisibilityRefresh();
                ...
                WCell.RealmServer.Asda2Quests.Asda2QuestHandler.SetQuests(this);
                ...
                Asda2InventoryHandler.FixSubUseItem(this);
                ...
                m_record.GetOrLoadFastItemSlots();
                ...
                {
                    IRealmClient delayedClient = Client;
                    Asda2PlayerInventory delayedInventory = Asda2Inventory;
                    if (delayedClient == null || !delayedClient.IsConnected ||
                        delayedClient.ActiveCharacter != this || IsLoggingOut ||
                        IsDisposed || delayedInventory == null)
                        return;
                    ...
                    Asda2CharacterHandler.SendRates(this);

                    if (MapId == MapId.Guildwave)
                    {
                        var guildWave = Asda2GuildWaveMgr.GetGuildWaveForId((int)GuildId);

                        if (guildWave != null)
                        {
                            if (guildWave.IsInRegisteringPlayer(this))
                            {
                                guildWave.PostAddEnteredPlayer(this);
                            }
                            else if (!GodMode)
                            {
                                WCell.RealmServer.Asda2GuildWave.Asda2GuildWave.TeleportToExitLocation(this);
                            }
                        }
                        else if (!GodMode)
                        {
                    ...
                    Asda2Battleground currentBattleGround = CurrentBattleGround;
                    if (currentBattleGround != null && currentBattleGround.IsRunning && MapId == MapId.BatleField)
                    {
                        IsChangingAsda2BattleGroundMap = false;
                        Asda2BattlegroundHandler.SendInitialWarState(this);
                    }
                    if (CurrentBattleGround == null && MapId == MapId.BatleField)
                    {
                        TeleportTo((IWorldLocation)LocatonBeforeOnEnterWar ?? BindLocation);
                    }
                    ...
                    if (TeleportPoints.Count(c => c != null) > 0 && Map != null)
                    {
                        Character loggedInCharacter = this;
                        Map.CallDelayed(1000, () =>
                        {
                            IRealmClient loginClient = loggedInCharacter.Client;
                            if (loginClient == null || !loginClient.IsConnected ||
                                loginClient.ActiveCharacter != loggedInCharacter)
                                return;
                            Asda2TeleportHandler.SendSavedLocationsInitResponse(loginClient);
                        });
                    }
                    if (delayedInventory.DonationItems != null &&
                        delayedInventory.DonationItems.Values.Any(di => di != null && !di.Recived))
                        Asda2InventoryHandler.SendSomeNewItemRecivedResponse(delayedClient, 20551, 102);
                    FunctionalItemsHandler.SendWingsInfoResponse(this, delayedClient);
                    foreach (KeyValuePair<Asda2ItemCategory, FunctionItemBuff> premiumBuff in PremiumBuffs)
                    {
                        for (int index = 0; index < (int)premiumBuff.Value.Stacks; ++index)
                            FunctionalItemsHandler.SendShopItemUsedResponse(delayedClient, premiumBuff.Value.ItemId,
                      ...
                      .FindAllByProperty("CharacterName", Name).Where(r => !r.IsDelivered).ToList())
                    {
                        ...
                        ServerApp<RealmServer>.IOQueue.AddMessage(() =>
                          delayedInventory.AddDonateItem(Asda2ItemMgr.GetTemplate(CharacterFormulas.DonationItemId), record.Amount,
                ...
                if (IsFirstLogin)
                {
                    if (m_record.MaxWeight <= 0)
                    {
                        m_record.MaxWeight = 11400;
                    }
                    MaxWeight = m_record.MaxWeight;
        ...
        private void ScheduleWorldEnterVisibilityRefresh()
        {
            if (Map == null)
                return;
            Map.CallDelayed(500, RefreshWorldEnterVisibilityForKnownViewers);
        }

        private void RefreshWorldEnterVisibilityForKnownViewers()
        {
            if (!IsInWorld || Map == null || Client == null)
                return;
            foreach (Character viewer in this.GetNearbyCharacters(BroadcastRange, false))
            {
                if (viewer == null || viewer == this || !viewer.IsInWorld || viewer.Client == null)
                    continue;
                if (viewer.Client.Channel != Client.Channel || !viewer.KnowsOf(this))
                    continue;
                Asda2MovmentHandler.SendStartMoveCommonToOneClienResponset(this, viewer.Client, true);
                GlobalHandler.SendCharacterVisibleNowResponse(viewer.Client, this);
            }
        }
        ...
        {
            int titleIndex = (int)titleId;
            if (!Asda2TitleTemplate.IsValidTitleId(titleIndex) || Asda2TitleTemplate.GetTemplate(titleIndex) == null)
                return;
            if (DiscoveredTitles == null)
                return;
            if (DiscoveredTitles.GetBit(titleIndex))
                return;
            DiscoveredTitles.SetBit(titleIndex);
            Asda2TitlesHandler.SendTitleDiscoveredResponse(Client, (short)titleIndex);
        }

        public void LearnTutorial(int tutorialId)
        {
            if (LearnedTutorials.GetBit(tutorialId))
            {
                Console.WriteLine($"Found ID: {tutorialId}");
            }

            if (LearnedTutorials.GetBit(tutorialId))
                return;
            LearnedTutorials.SetBit(tutorialId);
        }

        public void GetTitle(Asda2TitleId titleId)
        {
            int titleIndex = (int)titleId;
            Asda2TitleTemplate titleTemplate = Asda2TitleTemplate.GetTemplate(titleIndex);
            if (titleTemplate == null || GetedTitles == null)
                return;
            if (GetedTitles.GetBit(titleIndex))
                return;
            UpdateTitleCollectionProgress();
            GetedTitles.SetBit(titleIndex);
            Asda2TitlePoints += titleTemplate.Points;
            m_record.TitlePoints = (uint)Asda2TitlePoints;
            Asda2TitlesHandler.SendYouGetNewTitleResponse(this, (short)titleIndex);
            Asda2TitlesHandler.SendGetedTitlesResponse(Client);
        }

        public bool isTitleGetted(Asda2TitleId titleId)
        {
            return GetedTitles != null && GetedTitles.GetBit((int)titleId);
        }

        public bool isTitleDiscovered(Asda2TitleId titleId)
        {
            return DiscoveredTitles != null && DiscoveredTitles.GetBit((int)titleId);
        }

        private static uint[] NormalizeTitleMask(uint[] mask)
        {
            uint[] normalizedMask = new uint[Asda2TitleTemplate.TitleBlockCount];
            if (mask == null)
                return normalizedMask;
            Array.Copy(mask, normalizedMask, Math.Min(mask.Length, normalizedMask.Length));
            return normalizedMask;
        }

        private void UpdateTitleCollectionProgress()
        {
            try
            {
            ...
            {
                LogUtil.WarnException(ex, "Failed to update title collection progress for character {0}.", Name);
            }
        }
        ...
        protected internal bool SaveNow()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (m_Map != null)
                    {
                        if (m_Map.Id == MapId.Guildwave)
                        {
                            Vector3 exitPosition = WCell.RealmServer.Asda2GuildWave.Asda2GuildWave.GetExitPosition();
                            m_record.PositionX = exitPosition.X;
                            m_record.PositionY = exitPosition.Y;
                            m_record.PositionZ = exitPosition.Z;
                            m_record.Orientation = Orientation;
                            m_record.MapId = WCell.RealmServer.Asda2GuildWave.Asda2GuildWave.ExitMapId;
                            m_record.InstanceId = 0;
                            m_record.Zone = ZoneId.AlpiaMain;
                        }
                        else
                        {
            ...
            try
            {
                try
                {
                    Account.AccountData = AccountDataRecord.SaveAccountData(Account.AccountData);
                }
                catch (Exception ex)
                {
                    LogUtil.WarnException(ex,
        ...
        internal void DoFinishLogout()
        {
            ...
            this.BlueCharges = 0;
            this.RedCharges = 0;
            Asda2SoulGuard.CheckSoulGuard(this);
            ForfeitAsda2Duel();
            if (loggingOut != null)
                loggingOut(this);
            World.RemoveCharacter(this);
        ...
        {
            var record = CharacterRecord.FindByAccountAndSlot(targetAccId, targetCharNumOnAcc);
            return record == null
                ? LegacyCharacterIdFromAccIdAndCharNum(targetAccId, targetCharNumOnAcc)
                : record.EntityLowId;
        }

        public static uint LegacyCharacterIdFromAccIdAndCharNum(int targetAccId, short targetCharNumOnAcc)
        {
        ...
        internal void UpdateEnvironment(HashSet<WorldObject> updatedObjects)
        {
            ...
            {
                DateTime now = DateTime.Now;
                bool refreshVisibleNpcs =
                    (now - LastNpcVisibilityRefresh).TotalMilliseconds >= NpcVisibilityRefreshDelayMillis;
                if (refreshVisibleNpcs)
                    LastNpcVisibilityRefresh = now;
                ...
                Observing.IterateEnvironment(BroadcastRange, obj =>
                {
                    ...
                    if (!Observing.CanSee(obj) && !ReferenceEquals(obj, this))
                        return true;
                    bool isKnownObject = KnownObjects.Contains(obj);
                    if (refreshVisibleNpcs && isKnownObject)
                    {
                        GameObject visibleNpc = obj as GameObject;
                        if (CanSendAsda2NpcVisible(visibleNpc))
                            GlobalHandler.SendNpcVisiableNowResponse(Client, visibleNpc);
                    }
                    ...
                    if (!isKnownObject)
                    {
                        ...
                        if (visibleChr != null && visibleChr != this)
                        {
                            ...
                            if (visibleChr.IsOnTransport)
                                Map.CallDelayed(400,
                                  () => FunctionalItemsHandler.SendShopItemUsedResponse(Client, visibleChr));
                        ...
                        else
                        {
                            ...
                            {
                                if (visibleNpc.Channel == Client.Channel)
        ...
        private bool CanSendAsda2NpcVisible(GameObject npc)
        {
            return npc != null &&
                   npc.GoId != GOEntryId.Portal &&
                   (!IsAsda2BattlegroundInProgress ||
        ...
        public override void Update(int dt)
        {
            ...
            if (m_isLoggingOut)
                m_logoutTimer.Update(dt);
            if (DateTime.Now.Subtract(new TimeSpan(0, 0, 0, 15)) > LastCharge)
            {
                bool change = false;
                if (GreenCharges > 0)
                {
                ...
                if (change)
                {
                    Asda2SpellHandler.SendSetSkiillPowersStatsResponse(this, false, 0);
                    Asda2SoulGuard.CheckSoulGuard(this);
                }
            }
            if (PrivateShop != null && PrivateShop.Owner == this && IsAsda2TradeDescriptionPremium && PrivateShop.Trading)
            {
                if (DateTime.Now.Subtract(new TimeSpan(0, 1, 0)) > LastStoreCall)
                {
                    ChatMgr.SendAnnouncementChatResponse(this, $"[{Asda2TradeDescription}] X:{(int)Asda2X}, Y:{(int)Asda2Y}");
                    LastStoreCall = DateTime.Now;
                }
            }
            ...
            FunctionalItemsHandler.ExpireFunctionalVehicleIfNeeded(this);
