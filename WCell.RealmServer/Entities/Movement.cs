...
namespace WCell.RealmServer.Entities
{
  public class Movement
  {
    ...
    {
      if(!m_moving || m_owner == null || m_owner.Map == null)
        return;
      UpdatePosition();
    }
    ...
    private bool CheckCollision()
    {
      ...
      if(!(vector3_1 != Vector3.Zero))
        return false;
      float collisionMoveDistance = vector3_1.Length();
      if(float.IsNaN(collisionMoveDistance) || float.IsInfinity(collisionMoveDistance) ||
         collisionMoveDistance <= m_owner.BoundingRadius / 2.0f)
        return false;
