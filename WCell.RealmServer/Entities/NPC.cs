...
namespace WCell.RealmServer.Entities
{
  ...
  public class NPC : Unit, IQuestHolder, IEntity
  {
    ...
    {
      get { return Movement != null && Movement.IsMoving; }
    }
    ...
    public override int Health
    {
      ...
      set
      {
        ...
        public void GotAttacked()
        {
            ...
            {
                _timer.Stop();
                _timer.Dispose(); // Dispose of the existing timer
                _timer = null;    // Set it to null after disposal
                LastAttackTime = 0;
                CheckLastAttackTime();
            }
            else
        ...
        public void CheckLastAttackTime()
        {
            if (LastAttackTime >= 30)
            {
                if (_timer != null)
                {
                    _timer.Stop();
                ...
                LastAttackTime = 0;
                int HealthToAdd  = Convert.ToInt16((MaxHealth * 0.4));
                Health = HealthToAdd > MaxHealth ? MaxHealth : (Health + HealthToAdd);
                var nearby = World.GetAllCharacters().Where((chr) => chr.Asda2X <= Asda2X + 150 && chr.Asda2Y <= Asda2Y + 150);
                foreach (var chr in nearby)
                {
                    GlobalHandler.SendMonstrVisibleNowResponse(chr.Client, this);
                }
            }
            else
            {
        ...
        public Timer Timer { get { return _timer; } } // Expose _timer through property
      ...
      if(m_Map != null || this != null)
      {
        ...
        m_Map.CallDelayed(LastDamageDelay, () =>
        {
          Asda2CombatHandler.SendMostrDeadToAreaResponse(rcvrs, (short) UniqIdOnMap,
            ...
            var golditem = new Asda2LootItem(Asda2ItemMgr.GetTemplate(20551), (int)LootMoney * Convert.ToInt32(RealmServerConfiguration.Instance.Get("DefaultMoneyDropFactor")), 0);
            golditem.Loot = loot;
            if(loot != null)
            {
                if (loot.Items.Length > 0)
                {
                    int currentLength = loot.Items.Length;
                    Asda2LootItem[] newArray = new Asda2LootItem[currentLength + 1];
                    Array.Copy(loot.Items, newArray, currentLength);
                    newArray[currentLength] = golditem;
                    loot.Items = newArray;
                }
                else
                    loot.Items = new Asda2LootItem[] { golditem };
            }
            map.OnNPCDied(this);
          ...
          if(m_entry != null && Entry != null && Template != null)
          {
            if(EntryId == 510)
            {
              Map.GetObjectsInRadius<Character>(Position, 30, Phase, int.MaxValue).ForEach((c) =>
              {
                c.SendMonsterMsg(Name, "Ooh, You got me this time champ");
              });
            }
          ...
          EnterFinalState();
          WCell.RealmServer.Asda2Quests.Asda2QuestMgr.OnMonsterKilled(chr, this);
        });
      }
    ...
    private Character GetTopDamageCharacter(Group group)
    {
      if(group == null)
        return null;
      Character topCharacter = null;
      double topDamage = 0.0;
      foreach(KeyValuePair<object, double> damage in Damages)
      {
        Character chr = damage.Key as Character;
        if(chr == null || chr.Group != group ||
           topCharacter != null && damage.Value <= topDamage)
          continue;
        topCharacter = chr;
        topDamage = damage.Value;
      }
    ...
    private Character CalcLooter()
    {
      try
      {
        ...
        Group group = key1 as Group;
        if(group != null)
          character = GetTopDamageCharacter(group) ??
                      (group.Leader != null ? group.Leader.Character : null);
        Character chr1 = key1 as Character;
        if(chr1 != null)
          character = chr1;
        if(Entry.Rank >= CreatureRank.Boss)
        {
    ...
    protected internal override void OnResurrect()
    {
      ...
      if(m_spawnPoint != null && !m_spawnPoint.Pool.SpawnedObjects.Contains(this))
    ...
    public override void Update(int dt)
    {
      ...
      if(EntryId == 510 && Health <= (MaxHealth * 0.4) && !msgSent)
      {
         Map.GetObjectsInRadius<Character>(Position, 30, Phase, int.MaxValue).ForEach((c) =>
         {
             c.SendMonsterMsg(Name, "Don't think you can defeat me yet, I still got more");
         });
                msgSent = true;
      }
