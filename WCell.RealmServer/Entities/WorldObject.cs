...
namespace WCell.RealmServer.Entities
{
    ...
    INamed, IContextHandler
  {
    ...
    public static float BroadcastRangeNpc = 50f;
    ...
    {
        var f = GetDistanceSq(ref pt);
        var v = f <= distance * (double) distance;
        Console.WriteLine($"DQ: {f} | D:{distance * (double)distance} | DQ < D: {v}");
        return v;
    }
    ...
    public virtual void SendPacketToArea(RealmPacketOut packet, bool includeSelf = true, bool addEnd = true,
      Locale locale = WCell.Core.Network.Locale.Any, float? radius = null, byte? channel = null)
    {
        if (Map == null || Thread.CurrentThread.ManagedThreadId != m_Map.CurrentThreadId)
            return;

        float num = radius.HasValue ? radius.GetValueOrDefault() : BroadcastRange;
        ...
        if (locale == WCell.Core.Network.Locale.Any)
        {
            for (int index = m_Map.Characters.Count - 1; index >= 0; --index)
            {
                ...
                if ((includeSelf || !Equals(character)) && num > (double)GetDistance(character))
                {
                    if (channel == null || character.Client.Channel == channel)
                    {
    ...
    public bool IsAsda2GuildWave { get; set; }
