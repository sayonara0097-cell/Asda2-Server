...
namespace WCell.RealmServer.Database
{
    ...
    IActivePetSettings
  {
    ...
    [Field("MaxWeight", NotNull = true)]private int _maxWeight;
    [Field("MaxWeightIncrease", NotNull = true)]private int _maxWeightIncrease;
    ...
    [Property(Length = 18, NotNull = true, Unique = true)]
    ...
    public int MaxWeightIncrease
    {
      get { return _maxWeightIncrease; }
      set { _maxWeightIncrease = value; }
    }
    ...
    public static bool DeleteCharacterData(uint charId)
    {
      return DeleteCharAccessories(charId);
    }
    ...
    public static void DeleteChar(uint charId)
    {
      ServerApp<RealmServer>.IOQueue.ExecuteInContext(() =>
      {
        ...
        uint guildId = GetGuildId(charId);
        if(guildId != 0U)
    ...
    private static bool DeleteCharAccessories(uint charId)
    {
        try
	    {
		    ...
		    DeleteCharacterQuestData(charId);
		    Asda2ItemRecord.DeleteAll("OwnerId = " + charId);
		    DeleteCharacterFastItemSlots(charId);
		    WCell.RealmServer.Handlers.Asda2TeleportingPointRecord.DeleteAll("OwnerId = " + charId);
		    WCell.RealmServer.Mounts.Asda2MountRecord.DeleteAll("OwnerId = " + charId);
    ...
    private static void DeleteCharacterQuestData(uint charId)
    {
      DeleteRowsByCharacterId("questrecord", "OwnerId", charId);
      DeleteRowsByCharacterId("asda2questrecords", "OwnerId", charId);
    }

    private static void DeleteCharacterFastItemSlots(uint charId)
    {
      DeleteRowsByCharacterId("asda2fastitemslot", "OwnerId", charId);
    }

    private static void DeleteRowsByCharacterId(string tableName, string columnName, uint charId)
    {
      string sql = string.Format("DELETE FROM {0} WHERE {1} = :charId",
        DatabaseUtil.Dialect.QuoteForTableName(tableName),
        DatabaseUtil.Dialect.QuoteForColumnName(columnName));

      ActiveRecordMediator.Execute(typeof(CharacterRecord), (session, instance) =>
      {
        session.CreateSQLQuery(sql).SetParameter("charId", (long) charId).ExecuteUpdate();
        return null;
      }, null);
    }
    ...
    public static CharacterRecord FindByAccountAndSlot(int accId, short charNum)
    {
      CharacterRecord[] characters = FindAllByProperty("Created", "AccountId", accId);
      foreach(CharacterRecord character in characters)
      {
        if(character.CharNum == (byte) charNum)
          return character;
      }
    ...
    [Property(Length = 8)]
    public uint[] LearnedTutorials { get; set; }
