...
namespace WCell.RealmServer.Database
{
  ...
  public class AccountDataRecord : ActiveRecordBase<AccountDataRecord>
  {
    ...
    {
      return FindOne((ICriterion) Restrictions.Eq("accountId", accountID));
    }
    ...
    {
      AccountDataRecord existingRecord = GetAccountData(accountID);
    ...
    public static AccountDataRecord SaveAccountData(AccountDataRecord accountData)
    {
      if(accountData == null)
        return null;

      AccountDataRecord existingRecord = GetAccountData(accountData.accountId);
      if(existingRecord == null)
      {
        accountData.Create();
        return accountData;
      ...
      existingRecord.DataHolder = accountData.DataHolder;
      existingRecord.TimeStamps = accountData.TimeStamps;
      existingRecord.SizeHolder = accountData.SizeHolder;
      existingRecord.SoulmateIntroduction = accountData.SoulmateIntroduction;
    ...
    [Property(Length = 127)]
    public string SoulmateIntroduction { get; set; }
  }
}
