...
namespace WCell.RealmServer.Chat
{
  ...
  public static class ChatMgr
  {
      ...
      else
      {
        ...
        client.ActiveCharacter.SendPacketToArea(
          CreateNormalChatMessagePacket(client.ActiveCharacter.Name, str, client.Locale,
            client.ActiveCharacter), true, false, flag ? Locale.Any : client.Locale, new float?(), client.Channel);
      }
    }
    ...
    public static void AnnChatRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      if(client.ActiveCharacter.ChatBanned)
      {
        client.ActiveCharacter.SendInfoMsg("لقد تم حظر محادثتك.");
        return;
      }
      ...
      using(RealmPacketOut response = SendAnnChatResponse(client, client.ActiveCharacter.Name, msg, client.Locale))
        World.Broadcast(response, true, Locale.Any);
    }
    ...
    public static void SendAnnouncementChatResponse(Character chr, string message)
    {
        using (RealmPacketOut realmPacketOut = new RealmPacketOut(RealmServerOpCode.AnnChatResponse))
        {
            realmPacketOut.WriteInt32(chr.AccId);
            realmPacketOut.WriteInt16(chr.SessionId);
            realmPacketOut.WriteByte(0);
            realmPacketOut.WriteFixedAsciiString(chr.Name, 20, Locale.Any);
            realmPacketOut.WriteAsciiString(message, Locale.Any);
            chr.SendPacketToArea(realmPacketOut, true, false, Locale.Any, new float?(), chr.Client.Channel);
        }
    }
    ...
    public static void WishperChatRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      else
      {
        ...
        if(character == null)
           SendWishperChatResponse(client, soulmate, (int)client.ActiveCharacter.AccId,
        client.ActiveCharacter.SessionId, name, msg, null, 0);
        ...
        else
        {
          ...
          SendWishperChatResponse(character.Client, soulmate, (int)client.ActiveCharacter.AccId,
    ...
    public static void SendWishperChatResponse(IRealmClient recieverClient, byte soulmate, int senderAccId,
      short rcvSessId, string sender, string msg, IRealmClient senderClient = null, byte status = 1)
    {
