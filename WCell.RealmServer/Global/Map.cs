...
namespace WCell.RealmServer.Global
{
  ...
  public class Map : IGenericChatTarget, IMapId, IContextHandler, IWorldSpace
  {
    ...
    private const int BackupClearLootIntervalMillis = 600000;
    ...
    protected Map()
    {
      ...
      CallDelayed(BackupClearLootIntervalMillis, ClearLoot);
    }
    ...
    {
      foreach(Asda2Loot loot in Loots.ToList())
      {
        RemoveLootFromWorld(loot);
      }
    }

    public void ClearLootOnGuildWaveNow(int guildId)
    {
      foreach(Asda2Loot loot in Loots.ToList())
      {
        RemoveLootFromWorldOnGuildWave(loot, guildId);
      }
    }
    ...
    public NPC GetNpcByUniqMapId(ushort id, byte channel)
    {
      NPC npc = GetNpcByUniqMapId(id);
      return npc != null && npc.Channel == channel ? npc : null;
    }

    public List<NPC> GetNpcsById(ushort id)
    {
            List<NPC> npcs = new List<NPC>();
            foreach (var npc in _asda2Npcs)
            {
                if(npc.Value.EntryId == id)
                {
                    npcs.Add(npc.Value as NPC);
                }
            }
      return npcs;
    }

    public List<NPC> GetNpcsById(ushort id, byte channel)
    {
        List<NPC> npcs = new List<NPC>();
        foreach (var npc in _asda2Npcs)
        {
            if(npc.Value.EntryId == id)
            {
                if(npc.Value is NPC && (npc.Value as NPC).Channel == channel)
                npcs.Add(npc.Value as NPC);
            }
        }
      return npcs;
    ...
    public float GetDistanceSqWarPoint(ref Vector3 pt, Vector3 p2)
    {
      float num1 = pt.X - p2.X;
      float num2 = pt.Y - p2.Y;
      return (float) (num1 * (double) num1 + num2 * (double) num2);
    }

    public Asda2WarPoint GetNearestWarPoint(ref Vector3 pos, List<Asda2WarPoint> points)
    {
      EnsureContext();
      Asda2WarPoint point = null;
      float num = int.MaxValue;
      foreach(Asda2WarPoint p in points)
      {
        
          float distanceSq = GetDistanceSqWarPoint(ref pos, new Vector3(point.X, point.Y));
          if(distanceSq < (double) num)
          {
            num = distanceSq;
            point = (Asda2WarPoint) p;
          }
    ...
    private void DropLoot(Asda2Loot loot)
    {
      ...
      else
      {
        ...
        AddObjectLater(loot);
        ScheduleLootRemoval(loot);
      }
    }

    private void ScheduleLootRemoval(Asda2Loot loot)
    {
      int delayMillis = (int) (CharacterFormulas.DropLiveMinutes * 60000);
      if(delayMillis <= 0)
      {
        RemoveLootFromWorld(loot);
        return;
      ...
      CallDelayed(delayMillis, () => RemoveLootFromWorld(loot));
    }

    private void RemoveLootFromWorld(Asda2Loot loot)
    {
      if(loot == null || loot.Map != this || loot.Lootable == null || !Loots.Contains(loot))
        return;

      if(loot.Items != null)
      {
        foreach(Asda2LootItem item in loot.Items)
        {
          if(item == null)
            continue;

          if(IsLootSlotOccupiedBy(item))
          {
            GlobalHandler.SendRemoveItemResponse(item);
            ClearLootSlot(item);
          }
          else
          {
    ...
    private void RemoveLootFromWorldOnGuildWave(Asda2Loot loot, int guildId)
    {
      if(loot == null || loot.Map != this || loot.Lootable == null || !Loots.Contains(loot))
        return;
      ...
      if(loot.Items != null)
      {
        foreach(Asda2LootItem item in loot.Items)
        {
          ...
          if(IsLootSlotOccupiedBy(item))
          {
            GlobalHandler.SendRemoveItemOnGuildWaveResponse(item, guildId);
            ClearLootSlot(item);
          }
          else
          {
    ...
    private void ClearLootSlot(Asda2LootItem expectedItem)
    {
      if(!IsLootSlotOccupiedBy(expectedItem))
        return;
      expectedItem.Taken = true;
      _lootPositions[expectedItem.Position.X, expectedItem.Position.Y] = null;
    }

    private bool IsLootSlotOccupiedBy(Asda2LootItem item)
    {
      return item != null && IsValidLootSlot(item.Position.X, item.Position.Y) &&
             _lootPositions[item.Position.X, item.Position.Y] == item;
    }

    private bool IsValidLootSlot(short x, short y)
    {
      return x >= 0 && y >= 0 && x < _lootPositions.GetLength(0) && y < _lootPositions.GetLength(1);
    }
