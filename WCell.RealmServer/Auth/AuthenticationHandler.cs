...
namespace WCell.RealmServer.Auth
{
  ...
  public static class AuthenticationHandler
  {
    ...
    public static void ReturnToAuth(IRealmClient client, RealmPacketIn packet)
    {
      if(client == null)
        return;
      if(client.ActiveCharacter != null && !client.ActiveCharacter.IsInWorld)
      {
        if(client.Account != null)
          client.Account.ActiveCharacter = null;
        client.ActiveCharacter = null;
      }
      if(client.Account != null)
    ...
    public static void SendCharacterInfoLSResponse(IRealmClient client)
    {
      ...
      foreach(CharacterRecord character in client.AuthAccount.Characters)
      {
        ICollection<Asda2ItemRecord> loadedItems = character.GetOrLoadItems();
        using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.CharacterInfos))
        {
          ...
          {
            bool flag = false;
            foreach(Asda2ItemRecord asda2LoadedItem in loadedItems)
            {
              if(asda2LoadedItem != null && (int) asda2LoadedItem.Slot == index &&
                 asda2LoadedItem.InventoryType == (byte) Asda2InventoryType.Equipment)
              {
    ...
    public static void PingRequest(IRealmClient client, RealmPacketIn packet)
    {
        ...
        //    return;
        //if(DateTime.Now.Subtract(client.ActiveCharacter.LastPing).TotalMilliseconds < CharacterFormulas.NpcSpellUpdateDelay)
        //{
        ...
        //    else
        //    {
        //        if(client.Account.SetAccountActive(true, DateTime.MinValue))
        //        {
        //            if (client.ActiveCharacter.IsInWorld)
        //            {
        //                client.ActiveCharacter.Kick("Cheater", 300);
        //            }
        //        }
        //    }
        //}
