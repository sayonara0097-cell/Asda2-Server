...
namespace WCell.RealmServer.Network
{
  ...
  /// </summary>
  public interface IRealmClient : IClient, IDisposable, IPacketReceiver
  {
    /// <summary>The server this client is serviced by.</summary>
    new RealmServer Server { get; }

    bool IsGameServerConnection { get; set; }
    ...
    /// <summary>The current session key</summary>
    ...
    Auth.Accounts.Account AuthAccount { get; set; }

    /// <summary>Disconnects the client.</summary>
    /// <param name="disconnectSocket"> </param>
    void Disconnect(bool disconnectSocketWithDelay = false);
  }
}
