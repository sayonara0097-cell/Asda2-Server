...
namespace WCell.RealmServer.Asda2GuildWave
{
    class Asda2GuildWave
    {
        ...
        public const MapId ExitMapId = MapId.Alpia;
        private static readonly int[,] exitLocations =
        {
            {64, 339},
            {64, 341},
            {62, 341}
        ...
        public static Vector3 GetExitPosition()
        {
            Map map = World.GetNonInstancedMap(ExitMapId);
            float offset = map != null ? map.Offset : (float)((int)ExitMapId * 1000);
            return new Vector3(exitLocations[0, 0] + offset, exitLocations[0, 1] + offset, 0);
        }

        public static void TeleportToExitLocation(Character chr)
        {
            if (chr == null)
                return;
            ...
            lock (random)
            {
                index = random.Next(exitLocations.GetLength(0));
            }

            float? o = null;
            var pos = new Vector3(exitLocations[index, 0] + map.Offset, exitLocations[index, 1] + map.Offset, 0);
            chr.IsAsda2GuildWave = false;
            chr.TeleportTo(map, ref pos, o);
        }

        private void StartCurrentWave(bool notifyGuild)
        {
        ...
        private bool AreAllRegisteredPlayersDead()
        {
        ...
        private void EndGuildWaveAndExitPlayers(GuildWaveEnterStatus status, bool giveRewards)
        {
            List<Character> players;
            lock (_playersRegisteredLock)
            {
                players = playersRegistered.Where(chr => chr != null).ToList();
            }

            currentWaveNpcs.Clear();
            _map.CallOnAllNPCs(npc => npc.Kill());

            foreach (Character chr in players)
            {
                if (giveRewards)
                    GuildWaveResultAndItems(chr);
                TeleportToExitLocation(chr);
            }

            Asda2GuildWaveHandler.GuildWaveEnterStatusToGuild(_guild, status);
            terminating = true;
            _map.CallDelayed(5000, () =>
            {
                lock (_playersRegisteredLock)
                {
        ...
        public void ClearPlayersRegisteredList()
        {
            lock(_playersRegisteredLock)
                {
                    for (int i = playersRegistered.Count - 1; i >= 0; --i)
                    {
        ...
        public void UpdateGuildWave(int dt)
        {
            ...
            {
                EndGuildWaveAndExitPlayers(GuildWaveEnterStatus.Ended, false);
                return;
            }
        ...
        public void GuildWaveResultAndItems(Character chr)
        {
            ...
            AwardGuildWaveTitles(chr, LastWinnedWave);
        ...
        private static void AwardGuildWaveTitles(Character chr, int wave)
        {
            chr.DiscoverTitle(Asda2TitleId.Reserved140);
            if (wave >= 5)
            {
                chr.DiscoverTitle(Asda2TitleId.Reserved141);
            }
            if (wave >= 15)
            {
                chr.DiscoverTitle(Asda2TitleId.Reserved142);
            }
            if (wave >= 25)
            {
                chr.DiscoverTitle(Asda2TitleId.Reserved143);
            }
            if (wave > 15 && wave < 28)
            {
                chr.DiscoverTitle(Asda2TitleId.Reserved144);
            }
            if (wave > 28)
            {
                chr.DiscoverTitle(Asda2TitleId.Reserved145);
            }
        }
