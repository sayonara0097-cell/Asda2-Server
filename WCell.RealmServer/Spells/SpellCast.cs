  ...
  public class SpellCast : IUpdatable, IWorldLocation, IHasPosition
  {
    ...
    private void DelayedImpact(WorldObject obj)
    {
      ...
      foreach(WorldObject target in Targets.Where(
        target => !target.IsInWorld && (target as NPC).Channel == CasterChar.Client.Channel))
        Remove(target);
      try
      {
        int num = (int) Impact();
    ...
    public SpellFailedReason Impact()
    {
      if(!IsCasting)
        return SpellFailedReason.Ok;
      foreach(SpellEffectHandler handler in GetImpactHandlersInApplyOrder())
      {
    ...
    private IEnumerable<SpellEffectHandler> GetImpactHandlersInApplyOrder()
    {
      if(Spell != null && (Spell.Id == (uint) SpellId.FuriousChargeRank1 ||
                           Spell.RealId == (short) SpellLineId.GrandEarthquake))
        return Handlers.OrderBy(handler =>
          handler.Effect.EffectType == SpellEffectType.CastAnotherSpell ? 0 : 1);
      return Handlers;
    }
    ...
    protected void OnUnitCasted()
    {
      ...
      if(CasterUnit is Character)
      {
        CasterChar.Achievements.CheckPossibleAchievementUpdates(AchievementCriteriaType.CastSpell,
          Spell.Id, 0U, null);
        if(!IsPassive && !Spell.IsPassive)
          Asda2SoulGuard.CheckSoulGuard(CasterChar);
      }
      TriggerProcOnCasted();
    ...
    {
      if(Spell.RealId == (short) SpellLineId.LastResort)
      {
        CasterUnit.Power = 0;
        return;
      }
    ...
    private SpellFailedReason Start()
    {
      ...
      if(Spell.Effect0_ImplicitTargetA == ImplicitSpellTargetType.AllParty)
      {
        if(CasterChar != null && CasterChar.Group != null)
          InitialTargets = CasterChar.Group.GetAllCharacters(CasterChar.Client.Channel);
      }
      else
      {
    ...
    private SpellFailedReason FailStartWithoutSpell()
    {
      _log.Warn("{0} tried to cast without selecting a Spell.", CasterObject);
      if(Spell == null || !IsCasting && !IsChanneling)
        Cleanup();
      return SpellFailedReason.Error;
    }
    ...
    protected internal void Cleanup()
    {
      IsPlayerCast = false;
      IsCasting = false;
      if(Spell != null && Spell.IsTame && SelectedTarget is NPC)
