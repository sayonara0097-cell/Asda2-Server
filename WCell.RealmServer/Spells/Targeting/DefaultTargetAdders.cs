...
namespace WCell.RealmServer.Spells.Targeting
{
  ...
  public static class DefaultTargetAdders
  {
      ...
      ref SpellFailedReason failReason)
    {
      ...
      {
        log.Warn("Invalid SpellCast tried to target self, but no Caster given: {0}",
          targets.Cast);
        failReason = SpellFailedReason.Error;
      }
      else
      {
        if((sbyte) (failReason = targets.ValidateTargetForHandlers(casterObject)) != -1)
          return;
      ...
      ref SpellFailedReason failReason)
    {
      ...
      {
        log.Warn("Non-Player {0} tried to cast Pet - spell {1}", casterObject,
          targets.Cast.Spell);
      ...
      ref SpellFailedReason failReason, bool harmful)
    {
      ...
      if(target != casterObject && casterObject != null)
      {
        ...
        else if(casterObject.IsPlayer && !target.IsInFrontOf(casterObject))
      ...
      ref SpellFailedReason failReason)
    {
      ...
      {
        if((sbyte) (failReason = targets.ValidateTarget(casterUnit.ChannelObject, filter)) != -1)
          return;
    ...
    public static void AddAreaDest(this SpellTargetCollection targets, TargetFilter filter,
      ref SpellFailedReason failReason, float radius)
    {
      targets.AddTargetsInArea(IsGrandEarthquake(targets) ? targets.Cast.SourceLoc : targets.Cast.TargetLoc,
        filter, radius);
    }

    private static bool IsGrandEarthquake(SpellTargetCollection targets)
    {
      Spell spell = targets == null || targets.Cast == null ? null : targets.Cast.Spell;
      ...
      float radius)
    {
      ...
      int limit = spell.MaxTargetEffect == null
        ? (!(targetFilter == DefaultTargetFilters.IsAllied) ? (int) spell.MaxTargets : 40)
      ...
      ref SpellFailedReason failReason)
    {
      ...
      else
      {
        ...
        if(aggressorByThreatRank != null)
          targets.Add(aggressorByThreatRank);
        else
          failReason = SpellFailedReason.NoValidTargets;
      }
    }
