...
namespace WCell.RealmServer.Spells
{
    ...
    public abstract class SpellEffectHandler
    {
        ...
        public virtual void Apply()
        {
            if (m_targets != null)
            {
                ...
                for (CurrentTargetNo = 0; CurrentTargetNo < m_targets.Count; ++CurrentTargetNo)
                {
                    ...
                    if (Cast.Spell.TargetFlags == SpellTargetFlags.Unit)
                    {
                        if (target.IsInContext && ((target is NPC npcTarget && npcTarget.Channel == casterChar.Client.Channel) || (Cast.CasterObject is NPC casterNpc && target is Character chrTarget && chrTarget.Client.Channel == casterNpc.Channel)))
                        {
                            Apply(target, ref actions);
                            if (m_cast == null)
                                return;
                        }
                        else if ((Cast.CasterObject is Character casterChr && target is Character targetChr && targetChr.Client.Channel == casterChr.Client.Channel))
                        {
                            Apply(target, ref actions);
                            if (m_cast == null)
                                return;
                    ...
                    else if (Cast.Spell.TargetFlags == SpellTargetFlags.Self)
                    {
                    ...
                    else if (Cast.Spell.TargetFlags != SpellTargetFlags.Unit)
                    {
                        if (target.IsInContext)
                        {
                            Apply(target, ref actions);
                ...
                if (ShouldSuppressEmptySkillAnimation())
                    return;
                ...
                else
                {
                    ...
                    if (casterChar != null)
                    {
                        if (actions.Length > 0 && InitialTarget is Character targetChr)
                        {
                            foreach (var action in actions)
                            {
                                if (targetChr != null && action != null)
                                {
                                    if (targetChr.Damages == null)
                                        targetChr.Damages = new System.Collections.Generic.Dictionary<object, double>();
                                    if (targetChr.Damages.ContainsKey(casterChar))
                                    {
                                        var dmg = targetChr.Damages[casterChar];
                                        targetChr.Damages[casterChar] = dmg + action.ActualDamage;
                                    }
                                    else
                                    {
                                        targetChr.Damages[casterChar] = action.ActualDamage;
                                    }
                                }
                            }
                        }
        ...
        private bool ShouldSuppressEmptySkillAnimation()
        {
            return Effect != null && Effect.Spell != null &&
                   (Effect.Spell.Id == (uint)SpellId.Charge283Rank1FromFuriousCharge ||
                    IsGrandEarthquakeSilentAuxiliaryAura(Effect.Spell));
        }

        private static bool IsGrandEarthquakeSilentAuxiliaryAura(Spell spell)
        {
            return spell != null && spell.Id >= 856U && spell.Id <= 862U;
        }
