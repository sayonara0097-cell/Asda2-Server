...
namespace WCell.RealmServer.Spells
{
  ...
  public class NPCSpellCollection : SpellCollection
  {
    ...
    public void AddCooldown(Spell spell, int millis)
    {
      if(millis <= 0)
        return;
      m_readySpells.Remove(spell);
      if(m_cooldowns != null)
      {
        for(int index = m_cooldowns.Count - 1; index >= 0; --index)
        {
          CooldownRemoveTimer cooldown = m_cooldowns[index];
          if((int) cooldown.Spell.Id != (int) spell.Id)
            continue;
          Owner.RemoveUpdateAction(cooldown);
          m_cooldowns.RemoveAt(index);
        }
      }
      CooldownRemoveTimer cooldownRemoveTimer = new CooldownRemoveTimer(millis, spell);
      Owner.AddUpdateAction(cooldownRemoveTimer);
      if(m_cooldowns == null)
    ...
    protected class CooldownRemoveTimer : OneShotObjectUpdateTimer
    {
      ...
      {
        NPCSpellCollection spells = ((Unit) owner).Spells as NPCSpellCollection;
        if(spells == null)
          return;
        spells.AddReadySpell(Spell);
        if(spells.m_cooldowns != null)
          spells.m_cooldowns.Remove(this);
      }
    }
  }
}
