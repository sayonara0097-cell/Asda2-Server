...
namespace WCell.RealmServer.AI.Actions.Movement
{
    ...
    public class AITargetMoveAction : AIAction
    {
        ...
        public virtual bool IsInRange(WorldObject target)
        {
            ...
            {
                if (CanUseReadySpell(owner, readySpell, target) &&
                    (double) readySpell.Range.MaxDist * (double) readySpell.Range.MaxDist > (double) distanceSq)
                    return true;
            }
        ...
        protected static bool CanUseReadySpell(NPC owner, Spell spell, WorldObject target)
        {
            Unit unitTarget = target as Unit;
            if (unitTarget == null)
                return false;
            return spell.CheckCasterConstraints(owner) == SpellFailedReason.Ok &&
                   owner.CanCastSpellOn(unitTarget, spell) == SpellFailedReason.Ok &&
                   spell.CheckValidTarget(owner, target) == SpellFailedReason.Ok;
        }
