...
namespace WCell.RealmServer.AI.Actions.Combat
{
  ...
  public class AIAttackAction : AITargetMoveAction
  {
    ...
    protected bool TryCastSpell()
    {
      ...
      {
        if(owner.CastingTill >= DateTime.Now || !CanUseReadySpell(owner, readySpell, this.Target))
          continue;
        SpellFailedReason result = this.m_owner.SpellCast.Start(readySpell, this.Target);
        if(result == SpellFailedReason.Ok)
        {
          if(readySpell.CastDelay > 0)
            owner.CastingTill = DateTime.Now.AddMilliseconds((double) readySpell.CastDelay);
          return true;
        }

        owner.NPCSpells.AddCooldown(readySpell, NPCSpellCollection.DefaultNPCSpellCooldownMillis);
        return false;
      }
