...
namespace WCell.RealmServer
{
  ...
  public sealed class RealmServer : ServerApp<RealmServer>
  {
    ...
    private const int PendingReconnectLogoutDelayMillis = 15000;
    private const int PendingBattleGroundReconnectLogoutDelayMillis = 60000;
    ...
    protected override void OnClientDisconnected(IClient client, bool forced)
    {
      ...
      if(client1 != null && !client1.IsOffline)
      {
        ...
        if(account != null)
        {
          if(ReferenceEquals(account.Client, client1))
            account.Client = null;
          ...
          if(chr != null)
          {
            chr.AddMessage(() =>
            {
              if(!ReferenceEquals(chr.Client, client1))
                return;
              chr.IsConnected = false;
              if(chr.IsAsda2Teleporting || !client1.IsGameServerConnection)
              {
                int reconnectLogoutDelay = chr.IsChangingAsda2BattleGroundMap
                  ? PendingBattleGroundReconnectLogoutDelayMillis
                  : PendingReconnectLogoutDelayMillis;
                chr.IsAsda2Teleporting = false;
                chr.Logout(false, reconnectLogoutDelay);
              }
              else
              {
                chr.Logout(true, 0);
