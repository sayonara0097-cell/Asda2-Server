...
namespace WCell.RealmServer.Handlers
{
  public static class FunctionalItemsHandler
  {
    ...
    private const int FunctionalVehicleCooldownSeconds = 30;

    private static void SendFunctionalItemPacketToArea(Character chr, RealmServerOpCode opCode,
      Action<RealmPacketOut> writePacket)
    {
      if(chr == null)
        return;
      var map = chr.Map;
    ...
    private static void WriteShopItemUsedPacket(RealmPacketOut packet, Character trigger, int itemId,
      int durationSecs)
    {
    ...
    private static TimeSpan GetFunctionalVehicleCooldownRemaining(Character chr)
    {
      if(chr == null)
        return TimeSpan.Zero;
      TimeSpan remaining = chr.LastTransportUsedTime.AddSeconds(FunctionalVehicleCooldownSeconds) - DateTime.Now;
      return remaining > TimeSpan.Zero ? remaining : TimeSpan.Zero;
    }

    public static int GetFunctionalVehicleRemainingSeconds(Character chr, int itemId)
    {
      Asda2Item item = FindActiveFunctionalVehicle(chr, itemId);
      TimeSpan? remaining = GetRemainingFunctionalVehicleTime(item);
    ...
    public static int GetFunctionalVehicleRemainingHours(Character chr, int itemId)
    {
      Asda2Item item = FindActiveFunctionalVehicle(chr, itemId);
      TimeSpan? remaining = GetRemainingFunctionalVehicleTime(item);
      if(!remaining.HasValue)
      ...
      if(remaining.Value <= TimeSpan.Zero)
        return 0;
      return Math.Max(1, (int) Math.Ceiling(remaining.Value.TotalHours));
    }

    private static int GetRemainingShopItemHours(Asda2Item item)
    {
      TimeSpan? remaining = GetRemainingFunctionalVehicleTime(item);
      if(!remaining.HasValue)
        return -1;
    ...
    private static string GetFunctionalVehicleDurationText(Asda2Item item)
    {
      TimeSpan? remaining = GetRemainingFunctionalVehicleTime(item);
      int days = remaining.HasValue
        ? Math.Max(0, (int) Math.Ceiling(remaining.Value.TotalDays))
        : Math.Max(0, item == null ? 0 : item.AttackTime);
      if(days <= 0)
        return "اليوم";
      if(days >= 30)
        return string.Format("بعد {0} شهر", Math.Max(1, (int) Math.Ceiling(days / 30.0)));
      if(days >= 7)
        return string.Format("بعد {0} اسبوع", Math.Max(1, (int) Math.Ceiling(days / 7.0)));
      if(days == 1)
        return "بعد يوم";
      if(days == 2)
        return "بعد يومين";
      return string.Format("بعد {0} ايام", days);
    }

    private static void SendFunctionalVehicleDurationNotice(IRealmClient client, Asda2Item item)
    {
      if(client == null || client.ActiveCharacter == null || item == null)
        return;
      SendUpdateShopItemInfoResponse(client, UseFunctionalItemError.TheDurationOfTheShopitemHaExprised, item);
      GlobalHandler.SendServerMsg(client.ActiveCharacter,
        string.Format("المركبة ستصبح غير صالحة {0}", GetFunctionalVehicleDurationText(item)));
    }

    private static TimeSpan? GetRemainingFunctionalVehicleTime(Asda2Item item)
    {
      if(item == null || item.Record == null || item.Record.AuctionEndTime == DateTime.MinValue)
        return null;
      return item.Record.AuctionEndTime - DateTime.Now;
    }

    public static bool ExpireFunctionalVehicleIfNeeded(Character chr)
    {
      if(chr == null || !chr.IsOnTransport)
        return false;
      Asda2Item item = FindActiveFunctionalVehicle(chr, chr.TransportItemId);
      if(!IsFunctionalVehicleExpired(item))
        return false;
      RemoveExpiredFunctionalVehicle(chr, item, true);
      return true;
    }

    private static bool IsFunctionalVehicleExpired(Asda2Item item)
    {
      return item != null && item.Record != null && item.Record.AuctionEndTime != DateTime.MinValue &&
             DateTime.Now > item.Record.AuctionEndTime;
    }

    private static void RemoveExpiredFunctionalVehicle(Character chr, Asda2Item item, bool sendStatusPacket)
    {
      if(chr == null || item == null)
        return;
      if(chr.TransportItemId == item.ItemId)
        chr.TransportItemId = -1;
      if(sendStatusPacket && chr.Client != null && !chr.Client.IsOffline)
        SendUpdateShopItemInfoResponse(chr.Client, UseFunctionalItemError.TheDurationOfTheShopitemHaExprised,
          item);
      Asda2InventoryHandler.ItemRemovedFromInventoryResponse(chr, item, DeleteOrSellItemStatus.Ok, 0);
      item.Destroy();
    }

    private static Asda2Item FindActiveFunctionalVehicle(Character chr, int itemId)
    {
      if(chr == null || chr.Asda2Inventory == null)
        return null;
      Asda2Item item = chr.Asda2Inventory.ShopItems.FirstOrDefault(
        i => i != null && i.ItemId == itemId && i.Category == Asda2ItemCategory.IncMoveSpeed);
      if(item != null)
        return item;
      return chr.Asda2Inventory.RegularItems.FirstOrDefault(
        i => i != null && i.ItemId == itemId && i.Category == Asda2ItemCategory.IncMoveSpeed);
    }
    ...
    public static void SendShopItemUsedResponse(IRealmClient client, int itemId, int durationSecs = -1)
    {
      if(client == null || client.ActiveCharacter == null)
        return;
      Character trigger = client.ActiveCharacter;
      if(durationSecs < 0)
        durationSecs = GetFunctionalVehicleRemainingSeconds(trigger, itemId);
      SendFunctionalItemPacketToArea(trigger, RealmServerOpCode.ShopItemUsed,
        packet => WriteShopItemUsedPacket(packet, trigger, itemId, durationSecs));
    }

    public static void SendShopItemUsedResponse(IRealmClient rcv, Character trigger, int durationSecs = -1)
    {
      if(rcv == null || trigger == null)
        return;
      if(durationSecs < 0 || durationSecs == int.MaxValue)
        durationSecs = GetFunctionalVehicleRemainingSeconds(trigger, trigger.TransportItemId);
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.ShopItemUsed))
      {
        WriteShopItemUsedPacket(packet, trigger, trigger.TransportItemId, durationSecs);
    ...
    public static void UseTeleportScrollRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      if(character == null)
        client.ActiveCharacter.SendSystemMessage(string.Format("{0} is not in game.", name));
      else if(character.IsAsda2BattlegroundInProgress)
        client.ActiveCharacter.SendSystemMessage(string.Format("{0} is  on war.", name));
        ...
        switch(++progressRecord.Counter)
        {
          case 50:
            client.ActiveCharacter.Map.CallDelayed(500,
              () => client.ActiveCharacter.DiscoverTitle(Asda2TitleId.Stalker222));
            break;
          case 100:
            client.ActiveCharacter.Map.CallDelayed(500,
              () => client.ActiveCharacter.GetTitle(Asda2TitleId.Stalker222));
            break;
        }

        progressRecord.SaveAndFlush();
        if(flag)
          client.ActiveCharacter.TeleportTo(character);
        else
    ...
    public static void UseSummonScrollRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      else
      {
        ...
        switch(++progressRecord.Counter)
        {
          case 50:
            client.ActiveCharacter.Map.CallDelayed(500,
              () => client.ActiveCharacter.DiscoverTitle(Asda2TitleId.Lonely224));
            break;
          case 100:
            client.ActiveCharacter.Map.CallDelayed(500,
              () => client.ActiveCharacter.GetTitle(Asda2TitleId.Lonely224));
            break;
        }
    ...
    public static void UseShopItemRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      client.ActiveCharacter.AddMessage(() =>
        ProcessFunctionalItem(client, parametr, slot));
    }
    ...
    private static void ProcessFunctionalItem(IRealmClient client, uint parametr, short slot)
    {
      ...
      if(item == null)
      {
        SendUpdateShopItemInfoResponse(client,
          UseFunctionalItemError.FunctionalItemDoesNotExist, null);
      }
      else
      {
        UseFunctionalItemError status = UseFunctionalItemError.Ok;
        bool sendUpdateShopItemInfo = true;
            ...
            try
            {
              switch(item.Category)
              {
                case Asda2ItemCategory.IncPAtk:
                  ...
                  SendShopItemUsedResponse(client, item.ItemId,
                    item.Template.AtackRange);
                  break;
                ...
                case Asda2ItemCategory.IncMoveSpeed:
                  if(GetFunctionalVehicleCooldownRemaining(client.ActiveCharacter) > TimeSpan.Zero)
                  {
                  ...
                  if(IsFunctionalVehicleExpired(item))
                  {
                    RemoveExpiredFunctionalVehicle(client.ActiveCharacter, item, true);
                    status = UseFunctionalItemError.TheDurationOfTheShopitemHaExprised;
                    sendUpdateShopItemInfo = false;
                    break;
                  }

                  if(item.Record.AuctionEndTime == DateTime.MinValue)
                    item.Record.AuctionEndTime =
                      DateTime.Now + TimeSpan.FromDays(item.AttackTime);
                  ...
                  SendFunctionalVehicleDurationNotice(client, item);
                  sendUpdateShopItemInfo = false;
                ...
                case Asda2ItemCategory.ResetOneSkill:
                  Spell spell1 =
                    client.ActiveCharacter.Spells.First(
                      s => (long) s.RealId == (long) parametr);
                  if(spell1 != null)
                  {
                    ...
                    SendSkillResetedResponse(client, spell1.RealId,
                ...
                case Asda2ItemCategory.TeleportToCharacter:
                  ...
                  client.ActiveCharacter.TeleportTo(teleportPoint.MapId,
                    new Vector3(teleportPoint.X, teleportPoint.Y));
                  ...
                  break;
                case Asda2ItemCategory.IncreaceWeightCapasity:
                    if (client.ActiveCharacter.Record.MaxWeightIncrease < 4)
                    {
                        client.ActiveCharacter.Record.MaxWeight += 3750;
                        client.ActiveCharacter.Record.MaxWeightIncrease++;
                        client.ActiveCharacter.MaxWeight = client.ActiveCharacter.Record.MaxWeight;
                        client.ActiveCharacter.Record.Save();
                    }
                    else
                    {
                        status = UseFunctionalItemError.YouCanOnlyUseAMaxOf4WeightIncreaseItems;
                    }
                    break;
                case Asda2ItemCategory.InstantRecover100PrcHP:
                  ...
                  client.ActiveCharacter.HealPercent(100, null, null);
                  break;
                ...
                case Asda2ItemCategory.RecoverHp10TimesByPrcOver30Sec:
                  PereodicAction pereodicAction = null;
                ...
                case Asda2ItemCategory.PetNotEatingByDays:
                  SendPremiumLongBuffInfoResponse(client,
                    ...
                    item.ItemId, (short) item.Template.PackageId);
                  client.ActiveCharacter.Map.CallDelayed(500,
                    () => client.ActiveCharacter.GetTitle(Asda2TitleId.Treat366));
                  break;
                case Asda2ItemCategory.RemoveDeathPenaltiesByDays:
                  SendPremiumLongBuffInfoResponse(client,
                    ...
                    item.ItemId, (short) item.Template.PackageId);
                  break;
                case Asda2ItemCategory.ChangeGender:
                  client.ActiveCharacter.Gender = (GenderType)((byte)client.ActiveCharacter.Gender % 2 + 1);
                  break;
                case Asda2ItemCategory.ReviveOnSamePositionWithoutExpLoss:
    ...
    public static void UseChangeFactionItemRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      else
      {
        --shopShopItem.Amount;
        client.ActiveCharacter.Asda2FactionId = -1;
        SendFactionResetedResponse(client, true, shopShopItem);
      }
    }
      ...
      Asda2Item funcItem = null)
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.UpdateShopItemInfo))
      {
        ...
        packet.WriteInt32(GetRemainingShopItemHours(funcItem));
        packet.WriteInt16(client.ActiveCharacter.Record.MaxWeight);
    ...
    public static void SendWarehouseSlotsExpandedResponse(IRealmClient client, bool isAvatar)
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.WarehouseSlotsExpanded))
      {
        ...
        packet.WriteInt16((isAvatar
                            ...
                            : client.ActiveCharacter.Record.PremiumWarehouseBagsCount + 1) * 30);
        client.Send(packet, true);
      }
    }
    ...
    public static void SendCancelCancelFunctionalItemResponse(IRealmClient client, short itemId)
    {
      if(client.ActiveCharacter == null)
        return;
      Character trigger = client.ActiveCharacter;
      SendFunctionalItemPacketToArea(trigger, RealmServerOpCode.CancelFunctionalItem, packet =>
      {
        packet.WriteInt16(trigger.SessionId);
        packet.WriteInt32(itemId);
      });
      ...
      short itemCategory)
    {
      ...
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.PremiumLongBuffInfo))
      {
        ...
        packet.WriteInt32(
          (int) (client.ActiveCharacter.LongTimePremiumBuffs[slot].EndsDate - DateTime.Now)
          .TotalSeconds);
        client.Send(packet, true);
      }
    }
    ...
    public static void ExpandPetBoxRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      {
        SendPetBoxExpandedResponse(client,
          ExpandPetBoxStatus.Fail, num);
