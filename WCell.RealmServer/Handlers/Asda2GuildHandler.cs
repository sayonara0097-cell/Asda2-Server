  ...
  public static class Asda2GuildHandler
  {
    ...
    public static void ImpeachmentVoteRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      else
      {
        ...
        client.ActiveCharacter.SendErrorMsg($"{accepted}");
        if (!accepted)
          return;
    ...
    public static void CreateGuildRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      else if(!Asda2CharacterHandler.IsNameValid(clanName))
      {
        ...
        SendGuildCreatedResponse(client, CreateGuildStatus.GuildNameAlreadyExist,
          null);
      else if(!PrepareCharacterForGuildCreation(client.ActiveCharacter))
        SendGuildCreatedResponse(client, CreateGuildStatus.YouAreInAnotherGuild,
          null);
        ...
        ServerApp<RealmServer>.IOQueue.AddMessage(() =>
        {
          try
          {
            if(!PrepareCharacterForGuildCreation(client.ActiveCharacter))
            {
              client.ActiveCharacter.AddMoney(10000U);
              client.ActiveCharacter.SendMoneyUpdate();
              SendGuildCreatedResponse(client, CreateGuildStatus.YouAreInAnotherGuild, null);
              return;
            }

            Guild guild = new Guild(client.ActiveCharacter.Record, clanName);
            if(guild.Leader == null || guild.Leader.Character == null)
            {
              client.ActiveCharacter.AddMoney(10000U);
              client.ActiveCharacter.SendMoneyUpdate();
              SendGuildCreatedResponse(client, CreateGuildStatus.YouCantCreateAnotherGuild, null);
              return;
            }
    ...
    private static bool PrepareCharacterForGuildCreation(Character character)
    {
      try
      {
        if(character == null)
        ...
        GuildMember[] oldMembers = GuildMember.FindAllByProperty("CharacterLowId",
          (long) character.EntityId.Low);
        foreach(GuildMember oldMember in oldMembers)
        {
          Guild oldGuild = GuildMgr.GetGuild(oldMember.GuildId);
          if(oldGuild != null && oldGuild.Members.ContainsKey(oldMember.Id))
            return false;

          Singleton<GuildMgr>.Instance.UnregisterGuildMember(oldMember);
          oldMember.DeleteAndFlush();
        }

        character.Record.GuildId = 0U;
        character.SaveLater();
        return true;
      }
      catch(Exception)
    ...
    public static void SendGuildMembersInfoResponse(IRealmClient client, Guild guild)
    {
      ...
      foreach(GuildMember guildMember in guild.GetMembers()
                .Where(member => member != null)
                .GroupBy(member => member.Id)
                .Select(group => group.First())
                .OrderBy(member => member.RankId)
      ...
      foreach(List<GuildMember> guildMemberList2 in guildMemberListList)
      {
        using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.GuildMembersInfo))
        {
          ...
          {
            GuildMember guildMember = index < guildMemberList2.Count ? guildMemberList2[index] : null;
            WriteGuildMemberInfo(packet, guildMember);
          }
          client.Send(packet, false);
        }
      }
    ...
    private static void WriteGuildMemberInfo(RealmPacketOut packet, GuildMember guildMember)
    {
      if(guildMember == null)
      {
        packet.WriteInt32(0);
        ...
        packet.WriteInt32(0);
        packet.WriteByte(0);
        packet.WriteFixedAsciiString("", 17, Locale.Tahadi);
        packet.WriteByte(0);
        packet.WriteByte(0);
        packet.WriteFixedAsciiString("", 60, Locale.Start);
        packet.WriteInt32(0);
        return;
      }

      Character character = guildMember.Character;
      bool isOnline = IsGuildMemberOnline(guildMember);
      ...
      packet.WriteByte(isOnline ? character.ProfessionLevel : guildMember.ProffessionLevel);
      packet.WriteByte((byte) (isOnline ? character.Class : guildMember.Class));
      packet.WriteByte(4 - guildMember.RankId);
      packet.WriteInt32(isOnline ? character.GuildPoints : 0);
      packet.WriteByte(isOnline ? 1 : 0);
      packet.WriteFixedAsciiString(FormatGuildLastLogin(guildMember), 17, Locale.Tahadi);
      packet.WriteByte(isOnline ? 1 : 0);
      packet.WriteByte(isOnline ? (byte) character.MapId : (byte) 0);
      packet.WriteFixedAsciiString(guildMember.PublicNote, 60, Locale.Start);
      packet.WriteInt32(0);
    }

    public static void SendGuildMembersInfoToGuild(Guild guild)
    {
      if(guild == null)
        return;
      foreach(Character character in guild.GetCharacters())
      {
        if(character != null && character.Client != null)
          SendGuildMembersInfoResponse(character.Client, guild);
      }
    }

    private static string FormatGuildLastLogin(GuildMember guildMember)
    {
      DateTime lastLogin = guildMember == null ? DateTime.MinValue : guildMember.LastLogin;
      if(lastLogin <= DateTime.MinValue)
        return "";
      return lastLogin.ToString("yyyy-MM-dd HH:mm");
    }
    ...
    public static void LevelUpGuildRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      else if(client.ActiveCharacter.GuildMember == null ||
              !client.ActiveCharacter.GuildMember.HasRight(GuildPrivileges.UsePoints))
      {
    ...
    public static void JoinGuildResponseRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      else
      {
        ...
        GuildMember joinedMember = guild.AddMember(activeCharacter);
      ...
      Character rcv = null)
    {
      ...
      if(rcv != null)
      {
        using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.UpdateGuildInfo))
        {
          ...
          packet.WriteByte(guild.MemberCount);
          packet.WriteInt32(guild.Points);
          packet.WriteByte(guild.WaveLimit);
          packet.WriteByte(0);
          for(int index = 0; index < 10; ++index)
          {
    ...
    [PacketHandler(RealmServerOpCode.GuildNameChangeRequest)]
    public static void GuildNameChangeRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      Asda2Item changerItem = client.ActiveCharacter.Asda2Inventory.GetShopShopItem((short)slot);
      // 0 - fail | 2 - error in user file | 3 - Not in guild | 4 - Error with guild info | 5 - Not guild Leader | 6 - Bad name | 7 - Used before
      if(changerItem == null)
      {
          SendGuildNameChangedResponse(client, 0);
          return;
      }

      else if (!client.ActiveCharacter.IsInGuild)
      {
          SendGuildNameChangedResponse(client, 3);
          return;
      }

      else if(client.ActiveCharacter.Guild.Leader != client.ActiveCharacter.GuildMember)
      {
          SendGuildNameChangedResponse(client, 5);
          return;
      }

      else if(GuildMgr.GetGuild(clanName) != null)
      {
          SendGuildNameChangedResponse(client, 7);
          return;
      }

      //else if (!Asda2CharacterHandler.IsNameValid(clanName))
      //{
      //    SendGuildNameChangedResponse(client, 6);
      //    return;
      //}

      else
      {
          client.ActiveCharacter.Guild.Name = clanName;
          changerItem.Amount--;
          SendGuildNameChangedResponse(client, 1, changerItem);
          foreach (Character member in client.ActiveCharacter.Guild.GetCharacters())
          {
              SendClanFlagAndClanNameInfoSelfResponse(member);
          }
          ChatMgr.SendGlobalChatRemoveItemResponse(client, true, changerItem);
          Asda2InventoryHandler.UpdateItemInventoryInfo(client, changerItem);
      }
    ...
    public static void SendGuildNameChangedResponse(IRealmClient client, byte status, Asda2Item item = null)
        {
            using (RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.GuildNameChangeResponse))
            {
                packet.WriteByte(status);
                packet.WriteInt16(client.ActiveCharacter.GuildId);
                packet.WriteInt32(client.ActiveCharacter.AccId);
                packet.WriteInt16(client.ActiveCharacter.SessionId);
                packet.WriteFixedAsciiString(client.ActiveCharacter.Guild.Name, 17, Locale.Start);
                packet.WriteInt32(client.ActiveCharacter.Asda2GuildRank);
      ...
      GuildMember trigerer)
    {
      ...
      Character excludedCharacter =
        status == GuildNotificationType.Joined || status == GuildNotificationType.LoggedIn
          ? trigerer.Character
          : null;
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.GuildNotification))
      {
        ...
        bool isOnline = IsGuildMemberOnline(trigerer);
        packet.WriteByte(isOnline ? trigerer.Character.ProfessionLevel : trigerer.ProffessionLevel);
        packet.WriteByte((byte) (isOnline ? trigerer.Character.Class : trigerer.Class));
        packet.WriteByte(trigerer.Asda2RankId);
        packet.WriteInt32(isOnline ? trigerer.Character.GuildPoints : 0);
        packet.WriteByte(isOnline ? 1 : 0);
        packet.WriteFixedAsciiString(FormatGuildLastLogin(trigerer), 17, Locale.Tahadi);
        packet.WriteByte(isOnline ? 1 : 0);
        packet.WriteByte(isOnline ? (byte) trigerer.Character.MapId : (byte) 0);
        packet.WriteFixedAsciiString(trigerer.PublicNote, 60, Locale.Start);
        packet.WriteSkip(stab124);
        SendGuildPacket(guild, packet, true, Locale.Any, excludedCharacter);
      }
    }

    private static bool IsGuildMemberOnline(GuildMember member)
    {
      Character character = member == null ? null : member.Character;
      return character != null && character.Client != null && character.IsConnected && !character.IsLoggingOut;
    }

    private static void SendGuildPacket(Guild guild, RealmPacketOut packet, bool addEnd, Locale locale,
      Character excludedCharacter = null)
    {
      foreach(Character character in guild.GetCharacters())
      {
        if(character == null || character == excludedCharacter || character.Client == null)
          continue;
