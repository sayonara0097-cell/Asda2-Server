  ...
  internal class Asda2InventoryHandler
  {
    ...
    public static void ReplaceItemRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      if(status == Asda2InventoryError.Ok)
        return;
      SendItemReplacedResponse(client, status, num, srcInv, srcQuant, srcWeight,
        destSlot, destInv, destQuant, destWeight, false);
    }
    ...
    public static void SetFastItemSlotRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      for(byte panelSlot = 0; panelSlot < (byte) 12; ++panelSlot)
      {
        ...
        if(fastItemSlotRecords[panel][panelSlot] != null)
          fastItemSlotRecords[panel][panelSlot].DeleteLater();
        fastItemSlotRecords[panel][panelSlot] =
    ...
    public static void SendAllFastItemSlotsInfo(Character character)
    {
        ...
        .FastItemSlotRecords)
      {
        using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.FastItemSlotsInfo))
        {
          ...
          {
            packet.WriteByte(fastItemSlotRecord == null ? 0 : fastItemSlotRecord.SrcInfo);
            packet.WriteByte(fastItemSlotRecord == null ? 0 : fastItemSlotRecord.InventoryType);
            packet.WriteInt16(fastItemSlotRecord == null ? -1 : fastItemSlotRecord.InventorySlot);
    ...
    public static void UseItemRequest(IRealmClient client, RealmPacketIn packet)
    {
      try
      {
        ...
        Asda2InventoryType inventoryType = (Asda2InventoryType) num1;

        if((inventoryType != Asda2InventoryType.Regular && inventoryType != Asda2InventoryType.Shop) || num2 < 0)
          client.ActiveCharacter.SendInfoMsg("You must update your client to use items!1");
        else
          activeCharacter.Asda2Inventory.UseItem(inventoryType, (byte) num2);
      }
      catch(Exception)
      {
    ...
    public static void RemoveItemRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      if(inv == 4)
      {
        WCell.RealmServer.Asda2Quests.Asda2QuestHandler.RemoveQuestFromInventoryDeleteRequest(client, num1);
        return;
      }
      client.ActiveCharacter.Asda2Inventory.RemoveItem(num1, inv, num2);
    }
    ...
    public static void SendBuyItemResponse(Asda2BuyItemStatus status, Character chr, Asda2Item[] items)
    {
      if(items.Length != 7)
      {
        ...
        Array.Copy(asda2ItemArray, items,
    ...
    public static void UpdateItemInventoryInfo(IRealmClient client, Asda2Item item)
    {
      if(item.InventoryType == Asda2InventoryType.Regular)
      {
        ...
        {
          WriteItemInfoToPacket(packet, item, true);
      ...
      bool sendToGroup = true)
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.ItemPickuped))
      {
        ...
        WriteItemInfoToPacket(packet, item, false);
        packet.WriteInt16(chr.Asda2Inventory.Weight);
        if(sendToGroup && chr.IsInGroup)
            chr.Group.Send(packet, true);
        else
            chr.Client.Send(packet, true);
      }
    }

    public static void SendLootPickupFailedResponse(Asda2LootItem lootItem, Character chr,
      bool sendToGroup = true, Asda2PickUpItemStatus status = Asda2PickUpItemStatus.Fail)
    {
      if(lootItem == null || lootItem.Template == null)
      {
        SendItemPickupedResponse(status, null, chr, sendToGroup);
        return;
      }
      ...
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.ItemPickuped))
      {
        packet.WriteByte((byte) status);
        packet.WriteInt16(chr.SessionId);
        WriteLootItemInfoToPacket(packet, lootItem);
        packet.WriteInt16(chr.Asda2Inventory.Weight);
        if(sendToGroup && chr.IsInGroup)
          chr.Group.Send(packet, true);
        else
    ...
    public static void FixSubUseItem(Character chr)
    {
        if(chr == null || chr.Client == null || chr.Asda2Inventory == null)
            return;

        Asda2Item item = chr.Asda2Inventory.Equipment[10];
        if(item != null)
        {
            short s = (short)0;
            Asda2InventoryError status = chr.Asda2Inventory.TrySwap(Asda2InventoryType.Equipment, 10, Asda2InventoryType.Regular, ref s);
            if(status != Asda2InventoryError.Ok || s < 0 || s >= chr.Asda2Inventory.RegularItems.Length ||
               chr.Asda2Inventory.RegularItems[s] == null)
                return;

            short e = (short)10;
            Asda2InventoryHandler.UpdateItemInventoryInfo(chr.Client, chr.Asda2Inventory.RegularItems[s]);
            chr.Asda2Inventory.TrySwap(Asda2InventoryType.Regular, s, Asda2InventoryType.Equipment, ref e);
        }
    }
      ...
      if(upgradedItem != null)
      {
          ...
          switch(++progressRecord.Counter)
          {
            case 50:
              client.ActiveCharacter.Map.CallDelayed(500,
                () => client.ActiveCharacter.DiscoverTitle(Asda2TitleId.Upgraded255));
              break;
            case 100:
              client.ActiveCharacter.Map.CallDelayed(500,
                () => client.ActiveCharacter.GetTitle(Asda2TitleId.Upgraded255));
              break;
          }
        ...
        if(upgradedItem.Enchant == 10)
        {
          ...
          switch(++progressRecord.Counter)
          {
            case 25:
              client.ActiveCharacter.Map.CallDelayed(500,
                () => client.ActiveCharacter.DiscoverTitle(Asda2TitleId.Elite256));
              break;
            case 50:
              client.ActiveCharacter.Map.CallDelayed(500,
                () => client.ActiveCharacter.GetTitle(Asda2TitleId.Elite256));
              break;
          }
    ...
    public static void SoketAvatarRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      int num = packet.ReadInt16();
      short sowelCell = packet.ReadInt16();
      client.ActiveCharacter.Asda2Inventory.SowelItem(itemCell, sowelCell, sowelSlot, -1, true);
    }
      ...
      bool setAmountTo0WhenDeleted = false)
    {
      ...
      packet.WriteInt16(item == null ? -1 : item.Slot);
      packet.WriteInt16(item == null ? -1 : (item.IsDeleted ? -1 : 0));
      packet.WriteItemAmount(item, setAmountTo0WhenDeleted);
      packet.WriteByte(item == null ? -1 : item.Durability);
      packet.WriteInt16(item == null ? -1 : item.Weight);
      ...
      packet.WriteByte(item == null ? -1 : item.Enchant);
      //packet.WriteSkip(Stab31);
      packet.WriteInt16(0);
      packet.WriteByte(item == null ? -1 : item.EnchantResetCount);
      packet.WriteByte(item == null ? -1 : item.SealCount);
      ...
      packet.WriteInt16(item == null ? -1 : (short) item.Parametr3Type);
      packet.WriteInt16(item == null ? -1 : item.Parametr3Value);
      packet.WriteInt16(item == null ? -1 : (short) item.Parametr4Type);
      ...
      else
      {
        ...
        packet.WriteInt16(item == null ? -1 : (short) item.Parametr5Type);
    ...
    private static void WriteLootItemInfoToPacket(RealmPacketOut packet, Asda2LootItem lootItem)
    {
      Asda2ItemTemplate template = lootItem.Template;
      bool isMoney = (int) template.ItemId == 20551 || template.Id == 20551;
      int amount = lootItem.Amount > 0 ? lootItem.Amount : 1;

      packet.WriteInt32((int) template.ItemId);
      packet.WriteByte((byte) (template.IsShopInventoryItem ? Asda2InventoryType.Shop : Asda2InventoryType.Regular));
      packet.WriteInt16(-1);
      packet.WriteInt16(0);
      packet.WriteInt32(template.IsStackable || isMoney ? amount : 0);
      packet.WriteByte(template.MaxDurability);
      packet.WriteInt16(template.Weight);
      packet.WriteInt16(template.DefaultSoul1Id);
      packet.WriteInt16(0);
      packet.WriteInt16(0);
      packet.WriteInt16(0);
      packet.WriteByte(0);
      ...
      packet.WriteByte(0);
      packet.WriteByte(0);
      packet.WriteInt16((short) Asda2ItemBonusType.None);
      packet.WriteInt16(0);
      packet.WriteInt16((short) Asda2ItemBonusType.None);
      packet.WriteInt16(0);
      packet.WriteInt16((short) Asda2ItemBonusType.None);
    ...
    {
        packet.Position += 8;
        byte num1 = (byte)(packet.ReadByte() + 1);
        packet.Position -= 3;
      ...
      if(shopShopItem1 == null || !shopShopItem1.Template.IsEquipment && !shopShopItem1.Template.IsAvatar)
      {
        ...
        SendSowelRemovedResponse(client, SowelRemovedStatus.Fail, null,
          null, null);
      }
      else
      {
        ...
        else
        {
          ...
          switch(num1)
          {
            case 2:
              ...
              int num2 = (int) client.ActiveCharacter.Asda2Inventory.TryAdd(shopShopItem1.Soul2Id, 1,
                true, ref sowel, new Asda2InventoryType?(), null);
          ...
          SendSowelRemovedResponse(client, SowelRemovedStatus.Ok, shopShopItem1, sowel,
            shopShopItem2);
        }
      }
    }
    ...
    public static void OpenPackageRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      client.ActiveCharacter.Map.AddMessage(() =>
      {
        if(result == OpenPackageStatus.Ok)
          return;
        SendOpenPackageResponseResponse(client, result, null,
          packageInv, packageSlot, 0);
      });
    }
    ...
    public static void DisasembleEquipmentRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      client.ActiveCharacter.Map.AddMessage(() =>
      {
        if(status == DisasembleItemStatus.Ok)
          return;
        SendEquipmentDisasembledResponse(client, status, 0, null,
          slot);
      });
    }
      ...
      short weight, Asda2Item addedItem, short srcSlot)
    {
      ...
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.EquipmentDisasembled))
      {
        ...
        WriteItemInfoToPacket(packet, addedItem, false);
        packet.WriteInt16(addedItem == null ? 0 : addedItem.Weight);
        client.Send(packet, false);
      }
    }
      ...
      ServerApp<RealmServer>.IOQueue.AddMessage(() =>
      {
        ...
        client.ActiveCharacter.Map.AddMessage(() =>
        {
          if(status == BuyFromWarShopStatus.Ok)
            return;
          SendItemFromWarshopBuyedResponse(client, status, 0, 0,
            null, null);
        });
      });
    }
    ...
    public static void ShowWarehouseRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      if(num == 0 || num - 1 > client.ActiveCharacter.Record.PremiumWarehouseBagsCount)
        client.ActiveCharacter.YouAreFuckingCheater("Is trying to show premium warehouse bags that not owned.",
          50);
      else
        SendShowWarehouseItemsResponse(client, (byte) (num - 1U), false);
    }
    ...
    {
      byte num = packet.ReadByte();
      if(num == 0 || num - 1 > client.ActiveCharacter.Record.PremiumAvatarWarehouseBagsCount)
        client.ActiveCharacter.YouAreFuckingCheater("Is trying to show premium warehouse bags that not owned.",
          50);
      else
        SendShowWarehouseItemsResponse(client, (byte) (num - 1U), true);
    }
    ...
    {
      IEnumerable<Asda2WhItemStub> itemStubs = ReadItemStubs(packet);
    ...
    private static IEnumerable<Asda2WhItemStub> ReadItemStubs(RealmPacketIn packet)
    {
      ...
      return asda2WhItemStubArray
        .Where(itemStub => itemStub.Slot != (short) -1)
        .ToArray();
    }
    ...
    {
      IEnumerable<Asda2WhItemStub> asda2WhItemStubs = CreateArrayOfFiveElementsFromEnumerable(sourceItemStubs)
        .Concat(CreateArrayOfFiveElementsFromEnumerable(destItemStubs));
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.ItemsPushedToWarehouse))
      {
        ...
        if(status == PushItemToWhStatus.Ok)
        {
          ...
          {
            packet.WriteInt32(asda2WhItemStub == null ? -1 : asda2WhItemStub.Slot);
            packet.WriteByte(asda2WhItemStub == null ? 0 : (byte) asda2WhItemStub.Invtentory);
            packet.WriteInt32(asda2WhItemStub == null ? -1 : asda2WhItemStub.Amount);
            packet.WriteInt16(asda2WhItemStub == null ? 0 : asda2WhItemStub.Weight);
          }

          packet.WriteInt16(client.ActiveCharacter.Asda2Inventory.Weight);
          packet.WriteInt16(client.ActiveCharacter.Asda2Inventory.WarehouseItems.Where((item) => item != null).Count());
        }
      ...
      IEnumerable<Asda2WhItemStub> sourceItemStubs = null, IEnumerable<Asda2WhItemStub> destItemStubs = null)
    {
      ...
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.AvatarItemsStored))
      {
        ...
        if(status == PushItemToWhStatus.Ok)
        {
          ...
          packet.WriteInt16(client.ActiveCharacter.Asda2Inventory.Weight);
          packet.WriteInt16(client.ActiveCharacter.Asda2Inventory.AvatarWarehouseItems.Where((item) => item != null).Count());
        }
        client.Send(packet, false);
      }
    }
      ...
      IEnumerable<Asda2WhItemStub> sourceItemStubs = null, IEnumerable<Asda2WhItemStub> destItemStubs = null)
    {
      ...
      IEnumerable<Asda2WhItemStub> asda2WhItemStubs =
        CreateArrayOfFiveElementsFromEnumerable(destItemStubs).Concat(elementsFromEnumerable);
    ...
    public static void SendShowWarehouseItemsResponse(IRealmClient client, byte page, bool isAvatar)
    {
      ...
      Asda2Item[] asda2ItemArray = isAvatar
        ? asda2Inventory.AvatarWarehouseItems
          .Skip(page == byte.MaxValue ? 0 : page * 30)
          .Take(page == byte.MaxValue ? 270 : 30)
          .Where(it => it != null).ToArray()
        : asda2Inventory.WarehouseItems
          .Skip(page == byte.MaxValue ? 0 : page * 30)
          .Take(page == byte.MaxValue ? 270 : 30)
          .Where(it => it != null).ToArray();
      while(count < asda2ItemArray.Length)
      {
        asda2ItemListList.Add(new List<Asda2Item>(asda2ItemArray
          .Skip(count).Take(10)));
        count += 10;
      }
    ...
    public static void DisassembleAvatarRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      if(shopShopItem == null)
      {
        ...
        SendAvatarDissasembledResponse(client, AvatarDisassembleStatus.Fail, 0, 0,
          null);
      }
      else
      {
        ...
        AvatarDisasembleRecord disasembleRecord = null;
        if(flag)
        {
        ...
        if(disasembleRecord == null)
        {
          ...
          SendAvatarDissasembledResponse(client, AvatarDisassembleStatus.Fail, 0,
            0, null);
        }
        else
        {
          ...
          {
            client.ActiveCharacter.SendInfoMsg("Error " + asda2InventoryError);
            SendAvatarDissasembledResponse(client, AvatarDisassembleStatus.Fail, 0,
              0, null);
          }
          else
          {
            ...
            SendAvatarDissasembledResponse(client, AvatarDisassembleStatus.Ok,
    ...
    public static void StartAvatarSynthesisRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      Asda2Item shopShopItem2 = client.ActiveCharacter.Asda2Inventory.GetShopShopItem(slotInq2);
      Asda2Item suplItem =
        num == 0 ? null : client.ActiveCharacter.Asda2Inventory.GetShopShopItem(slotInq3);
      ...
      else
      {
          ...
          shopShopItem2 == null ? Asda2ItemQuality.Orange : shopShopItem2.Template.Quality))
        {
          ...
          SendAvatarSynthesisResultResponse(client, AvatarSyntesStatus.Ok,
            shopShopItem1, shopShopItem3 ?? shopShopItem2, suplItem);
        }
        else
          SendAvatarSynthesisResultResponse(client, AvatarSyntesStatus.Fail,
            shopShopItem1, shopShopItem3 ?? shopShopItem2, suplItem);
      }
    }
    ...
    public static void AdvacedEnchantWeaponRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          switch(shopShopItem.Template.AuctionLevelCriterion)
          {
            case AuctionLevelCriterion.Zero:
              if(regularItem1.Amount < 1 || regularItem2.Amount < 1 || regularItem3.Amount < 3)
              {
                ...
                SendAdvancedEnchantDoneResponse(client,
                  AdvancedEnchantStatus.NotEnoughtMaterials, null, null,
                  null, null);
                return;
              }
              ...
              if(!client.ActiveCharacter.SubtractMoney(50000U))
              {
                ...
                SendAdvancedEnchantDoneResponse(client,
                  AdvancedEnchantStatus.NotEnoghtGold, null, null,
                  null, null);
                return;
              }
          ...
          SendAdvancedEnchantDoneResponse(client, AdvancedEnchantStatus.Ok,
    ...
    public static void SealItemRequest(IRealmClient client, RealmPacketIn packet)
    {
        ...
        packet.Position += 3;
        var itemCell = packet.ReadInt16();
        var scrollId = packet.ReadInt16();

        Asda2Item item = client.ActiveCharacter.Asda2Inventory.GetShopShopItem(itemCell);
        Asda2Item scroll = client.ActiveCharacter.Asda2Inventory.GetShopItemById(scrollId);

        if (item == null || scroll == null)
        {
            SealItemResponse(client, 0, item, scroll);
            return;
        }

        // Inputs
        int rarity = (int)item.Template.Quality;           // rarity: 0..3 (per your data)
        int level = item.RequiredLevel;     // CHANGE THIS if your item level is stored elsewhere
        int sealCountBefore = item.Record.SealCount; // seal count BEFORE increment (no upper limit)

        if (sealCountBefore < 0)
        {
            SealItemResponse(client, 0, item, scroll);
            return;
        }

        int requiredScrolls = GetSealScrollsRequired(level, rarity, sealCountBefore);

        // Fail safe: unknown rarity/tier or not enough scrolls
        if (requiredScrolls <= 0 || scroll.Amount < requiredScrolls)
        {
            SealItemResponse(client, 0, item, scroll);
            return;
        }
        ...
        UpdateItemInventoryInfo(client, scroll);
        UpdateItemInventoryInfo(client, item);

        SealItemResponse(client, 1, item, scroll);
    }

    private struct SealCoeff
    {
        public int A; // base
        public int B; // per-seal linear increment
        public int C; // extra bump every 2 seals: floor((s+1)/2)

        public SealCoeff(int a, int b, int c)
        {
            A = a;
            B = b;
            C = c;
    ...
    /// <summary>
    /// Universal model validated from your tests:
    /// scrolls = A + B*s + C*floor((s+1)/2)
    /// where s is the current SealCount BEFORE increment.
    /// </summary>
    private static int GetSealScrollsRequired(int level, int rarity, int s)
    {
        // floor((s+1)/2) with integer division
        int halfBump = (s + 1) / 2;

        SealCoeff coeff = GetCoeff(level, rarity);

        // If coeff is unknown, return 0 to fail safe (no deduction/seal)
        if (coeff.A == 0 && coeff.B == 0 && coeff.C == 0)
            return 0;

        return coeff.A + (coeff.B * s) + (coeff.C * halfBump);
    }

    private static SealCoeff GetCoeff(int level, int rarity)
    {
        if (rarity == 0) return GetCoeffR0(level);
        if (rarity == 1) return GetCoeffR1(level);
        if (rarity == 2) return GetCoeffR2(level);
        if (rarity == 3) return GetCoeffR3(level);
        if (rarity == 4) return GetCoeffR4(level);

        // Unsupported rarity
        return new SealCoeff(0, 0, 0);
    }

    // ----------------------------
    // RARITY 0 (linear) - anchors from your tests:
    // L10: A1 B1, L20: A3 B3, L40: A9 B6, L60: A12 B9, L80: A24 B12
    // Uses nearest-lower-tier fallback for levels like 12, 25, 48, etc.
    // ----------------------------
    private static SealCoeff GetCoeffR0(int level)
    {
        if (level >= 80) return new SealCoeff(24, 12, 0);
        if (level >= 60) return new SealCoeff(12, 9, 0);
        if (level >= 40) return new SealCoeff(9, 6, 0);
        if (level >= 20) return new SealCoeff(3, 3, 0);
        if (level >= 10) return new SealCoeff(1, 1, 0);

        // Fallback for very low levels
        return new SealCoeff(1, 1, 0);
    }

    // ----------------------------
    // RARITY 1 (partially mapped) - anchors from your tests:
    // L10: (1,1,0), L20: (3,3,1), L40: (10,7,1), L60: (14,11,0), L80: (28,14,1)
    // Uses nearest-lower-anchor fallback until more tier samples are provided.
    // ----------------------------
    private static SealCoeff GetCoeffR1(int level)
    {
        if (level >= 80) return new SealCoeff(28, 14, 1);
        if (level >= 60) return new SealCoeff(14, 11, 0);
        if (level >= 40) return new SealCoeff(10, 7, 1);
        if (level >= 20) return new SealCoeff(3, 3, 1);
        if (level >= 10) return new SealCoeff(1, 1, 0);
    ...
    // ----------------------------
    // RARITY 2 (tiered) - confirmed from your tests:
    // L18: (1,1,1)
    // L20-38: (4,4,1)  (includes L20,28,38)
    // L42-55: (13,9,0) (includes L42,45,48,55)
    // L60-79: (18,14,-1) (includes L60,65)
    // L80+: (36,17,-1) - scaled for levels 80+
    // Gaps (39-41, 56-59) map down safely via nearest-lower-tier.
    // ----------------------------
    private static SealCoeff GetCoeffR2(int level)
    {
        if (level >= 80) return new SealCoeff(36, 17, -1);
        if (level >= 60) return new SealCoeff(18, 14, -1);
        if (level >= 42) return new SealCoeff(13, 9, 0);
        if (level >= 20) return new SealCoeff(4, 4, 1);
        if (level >= 18) return new SealCoeff(1, 1, 1);

        return new SealCoeff(1, 1, 1);
    }

    // ----------------------------
    // RARITY 3 (linear, tiered) - confirmed from your tests:
    // L18: (2,2,0)
    // L28-38: (6,6,0)
    // L48-55: (18,12,0)
    // L60-79: (24,18,0)
    // L80+: (48,21,0) - scaled for levels 80+
    // Gaps map down safely via nearest-lower-tier.
    // ----------------------------
    private static SealCoeff GetCoeffR3(int level)
    {
        if (level >= 80) return new SealCoeff(48, 21, 0);
        if (level >= 60) return new SealCoeff(24, 18, 0);
        if (level >= 48) return new SealCoeff(18, 12, 0);
        if (level >= 28) return new SealCoeff(6, 6, 0);
        if (level >= 18) return new SealCoeff(2, 2, 0);

        return new SealCoeff(2, 2, 0);
    }

    // ----------------------------
    // RARITY 4 (linear, C=0) - from reverse-engineered data:
    // L28 & L38: (9,9), L48: (27,18), L60: (36,27), L80: (72,36).
    // L18-27 & <18: (3,3) extrapolated.
    // ----------------------------
    private static SealCoeff GetCoeffR4(int level)
    {
        if (level >= 80) return new SealCoeff(72, 36, 0);
        if (level >= 60) return new SealCoeff(36, 27, 0);
        if (level >= 48) return new SealCoeff(27, 18, 0);
        if (level >= 28) return new SealCoeff(9, 9, 0);
        if (level >= 18) return new SealCoeff(3, 3, 0);

        return new SealCoeff(3, 3, 0);
    }

    public static void SealItemResponse(IRealmClient client, byte status, Asda2Item item, Asda2Item scroll)
    {
        using (RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SealItemResponse))
        {

            packet.WriteInt16(status); // 1 - 13
            packet.WriteInt16(client.ActiveCharacter.SessionId);
            packet.WriteInt32(client.ActiveCharacter.AccId);
            WriteItemInfoToPacket(packet, item, false);
            WriteItemInfoToPacket(packet, scroll, false);    
            client.Send(packet, true);
        }
    }
    ...
    public static void SummonBossRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      else
      {
        ...
        else if(items.Count < summonRecord.Amount)
        {
          client.ActiveCharacter.SendInfoMsg("not enought stones. required " + summonRecord.Amount);
        }
        else
        {
          ...
          if(entry == null)
          {
            client.ActiveCharacter.SendInfoMsg(string.Format("Summon record {0} has invalind npc Id {1}.",
              items[0].ItemId, summonRecord.MobId));
          }
          else
          {
            if(client.ActiveCharacter.MapId == summonRecord.MapId && client.ActiveCharacter.Map.GetNpcsById((ushort)entry.Entry.Id, client.Channel).Count > 0)
            {
                SendMonsterSummonedResponse(client, 0, items);
                return;
            }else{        
                WorldLocation worldLocation = new WorldLocation(summonRecord.MapId,
                  new Vector3(summonRecord.X + (int) summonRecord.MapId * 1000,
                    summonRecord.Y + (int) summonRecord.MapId * 1000), 1U);
                entry.SpawnAt(worldLocation, client.Channel, false).Brain.EnterDefaultState();
                SummonHelper(entry, worldLocation, client.Channel);
                SendMonsterSummonedResponse(client, 1, items);
                client.ActiveCharacter.GetNearbyCharacters(true).ForEach((c) =>
                {
                    if(entry.Id == 510)
                    c.SendMonsterMsg(entry.DefaultName, "Okay now, who called me here");
                });
            }
          }
        }
    ...
    public static void SummonHelper(NPCEntry entry, WorldLocation loc, byte channel)
    {
        switch (entry.Id)
        {
            case 508:
                SpawnMob(0, -3, 313, loc, channel);
                SpawnMob(3, 0, 313, loc, channel);
                SpawnMob(0, 3, 316, loc, channel);
                SpawnMob(-3, 0, 316, loc, channel);
                break;
            case 509:
                SpawnMob(0, -3, 313, loc, channel);
                SpawnMob(3, 0, 313, loc, channel);
                ...
                break;
            case 510:
                SpawnMob(0, -3, 324, loc, channel);
                SpawnMob(3, -3, 325, loc, channel);
                SpawnMob(3, 0, 326, loc, channel);
                SpawnMob(3, 3, 327, loc, channel);
                SpawnMob(0, 3, 324, loc, channel);
                SpawnMob(-3, 3, 325, loc, channel);
                SpawnMob(-3, 0, 326, loc, channel);
                SpawnMob(-3, -3, 327, loc, channel);
                break;
            case 512:
                SpawnMob(0, -3, 392, loc, channel);
                SpawnMob(3, 0, 392, loc, channel);
                SpawnMob(0, 3, 377, loc, channel);
                SpawnMob(-3, 0, 377, loc, channel);
                break;
            case 513:
                SpawnMob(0, -3, 392, loc, channel);
                SpawnMob(3, 0, 392, loc, channel);
            ...
            case 514:
                SpawnMob(0, -3, 392, loc, channel);
                SpawnMob(3, 0, 392, loc, channel);
                SpawnMob(0, 0, 377, loc, channel);
                SpawnMob(-3, 0, 377, loc, channel);
                break;
            case 515:
                SpawnMob(0, -3, 392, loc, channel);
                SpawnMob(3, -3, 392, loc, channel);
                SpawnMob(3, 0, 392, loc, channel);
                SpawnMob(3, 3, 392, loc, channel);
                SpawnMob(0, 3, 377, loc, channel);
                SpawnMob(-3, 3, 377, loc, channel);
                SpawnMob(-3, 0, 377, loc, channel);
                SpawnMob(-3, -3, 377, loc, channel);
                break;
            default:
    ...
    private static void SpawnMob(int x, int y, uint npcId, WorldLocation dest, byte channel)
    {
        Vector3 pos = new Vector3(dest.Position.X + x, dest.Position.Y + y);
        WorldLocation worldLocation = new WorldLocation(dest.Map, pos, 1U);
        NPCMgr.GetEntry(npcId).SpawnAt(worldLocation, channel, false).Brain.State = BrainState.Roam;
    }

    public static void SendMonsterSummonedResponse(IRealmClient client, byte status, List<Asda2Item> items)
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.MonsterSummoned))
      {
        ...
        for(int index = 0; index < 6; ++index)
        {
          Asda2Item asda2Item = items.Count <= index ? null : items[index];
    ...
    public static async void SendStartMinningResponse(Character chr, byte stage)
    {
        using (RealmPacketOut packet = new RealmPacketOut((RealmServerOpCode)6753))
        {
            packet.WriteByte(1);
            packet.WriteInt32(chr.Asda2Inventory.Weight);
            chr.Send(packet);
        }
        if(stage == 0)
        {
            await Task.Delay(TimeSpan.FromMilliseconds(100));
            SendStartMinningResponse(chr, 1);
        }
    }

    public static void SendMinningReslutResponse(Character chr, byte status)
    {
        using (RealmPacketOut packet = new RealmPacketOut((RealmServerOpCode)6754))
        {
            packet.WriteByte(status);
            packet.WriteInt32(chr.Asda2Inventory.Weight);
            chr.Send(packet);
    ...
    public static void SendStopMinningResponse(Character chr)
    {
        using (RealmPacketOut packet = new RealmPacketOut((RealmServerOpCode)6756))
        {
            packet.WriteByte(1);
            packet.WriteInt32(chr.Asda2Inventory.Weight);
            chr.Send(packet);
      ...
      else
      {
        ...
        else
        {
          ...
          int num3 = (int) client.ActiveCharacter.Asda2Inventory.TryAdd(donationItem.ItemId,
            donationItem.Amount, true, ref asda2Item, new Asda2InventoryType?(), null);
          Log.Create(Log.Types.ItemOperations, LogSourceType.Character, client.ActiveCharacter.EntryId)
            .AddAttribute("source", 0.0, "donation_mail")
            .AddAttribute("added_amount", donationItem.Amount, "").AddItemAttributes(asda2Item, "")
            .AddAttribute("donation_id", donationItem.Guid, "")
            .AddAttribute("creator", 0.0, donationItem.Creator).Write();
          if(asda2Item != null)
            asda2Item.IsSoulbound = true;
          SendItemFromMailTakedResponse(client, TakeItemMallItemsResult.Ok,
            new List<Asda2Item>
            {
              asda2Item
            });
    ...
    public static void ShowMeItemMallItemsRequest(IRealmClient client, RealmPacketIn packet)
    {
      List<Asda2DonationItem> list = client.ActiveCharacter.Asda2Inventory.DonationItems.Values
        .Take(7).ToList();
      SendItemMailListResponse(client, list);
    }
    ...
    public static void SendItemMailListResponse(IRealmClient client, List<Asda2DonationItem> items)
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.ItemMailList))
      {
        ...
        {
          Asda2DonationItem asda2DonationItem = items.Count <= val ? null : items[val];
