...
namespace WCell.RealmServer.Handlers
{
  internal class Asda2PvpHandler
  {
    ...
    public static void PvpRquestRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      else if(Asda2DigMgr.IsInTown(client.ActiveCharacter) ||
              Asda2DigMgr.IsInTown(characterBySessionId))
      {
        SendPvpTownBlockedResponse(client.ActiveCharacter,
      ...
      {
        SendPvpRequestToCharFromSrvResponse(client.ActiveCharacter, characterBySessionId);
    ...
    public static void AnswerPvpRequestRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      else
      {
        ...
        {
          Character opponent = client.ActiveCharacter.Asda2DuelingOponent;
          if(Asda2DigMgr.IsInTown(client.ActiveCharacter) ||
             Asda2DigMgr.IsInTown(opponent))
          {
            SendPvpTownBlockedResponse(client.ActiveCharacter,
              opponent);
            SendPvpTownBlockedResponse(opponent,
        ...
        {
          SendPvpStartedResponse(Asda2PvpResponseStatus.Rejected,
    ...
    public static void SendPvpTownBlockedResponse(Character rcv, Character opponent)
    {
      SendPvpStartedResponse(Asda2PvpResponseStatus.YouCantFightInTown, rcv, opponent);
    }
    ...
    public static void SendPvpRoundEffectResponse(Character firstDueler, Character secondDueler)
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.PvpRoundEffect))
      {
        ...
        packet.WriteInt16((short) ((firstDueler.Asda2X + (double) secondDueler.Asda2X) / 2.0));
        packet.WriteInt16((short) ((firstDueler.Asda2Y + (double) secondDueler.Asda2Y) / 2.0));
