...
using WCell.Util.Threading.ActorModel;
...
namespace WCell.RealmServer.Handlers
{
    public static class Asda2LoginHandler
    {
        ...
        private const byte ChannelCount = 3;
        ...
        public static void SelectServerRequest(IRealmClient client, RealmPacketIn packet1)
        {
            using (RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.ChanelInfoResponse))
            {
                WriteChanelInfo(packet, null);
                client.Send(packet, false);
            }
        }

        public static void SendChanelInfoResponse(IRealmClient client, byte channel)
        {
            using (RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.ChanelInfoResponse))
            {
                WriteChanelInfo(packet, channel);
                client.Send(packet, false);
            }
        }

        private static void WriteChanelInfo(RealmPacketOut packet, byte? selectedChannel)
        {
            int[] channelCounts = GetChannelCharacterCounts();
            for (int i = 0; i < ChannelCount; i++)
            {
                ...
                {
                    packet.WriteInt16(selectedChannel.HasValue && selectedChannel.Value != channel
                        ? -1
        ...
        private static int[] GetChannelCharacterCounts()
        {
            int[] counts = new int[ChannelCount];
            foreach (Character chr in World.GetAllCharacters())
            {
                if (chr == null || chr.Client == null || !chr.Client.IsGameServerConnection || !chr.IsConnected)
                    continue;
                if (chr.Client.Channel < ChannelCount)
                    counts[chr.Client.Channel]++;
            }
            return counts;
        }

        [PacketHandler(RealmServerOpCode.ChangeChannel)]
        public static void ChangeChannelRequest(IRealmClient client, RealmPacketIn packet)
        {
            byte channel = packet.ReadByte();
            Character character = client.ActiveCharacter;
            if (channel >= ChannelCount || character == null)
                return;

            if (client.Channel == channel)
            {
                ResetChannelBoundCharacterState(character);
                SendChanelInfoResponse(client, channel);
                return;
            }

            ResetChannelBoundCharacterState(character);
            client.Channel = channel;
            character.ForfeitAsda2Duel();
            character.SaveNow();
            character.ResetOwnWorld();
            Asda2CharacterHandler.ChangeLocationResponse(character, character.MapId, (short)character.Asda2X, (short)character.Asda2Y);
            SendChanelInfoResponse(client, channel);
        }

        private static void ResetChannelBoundCharacterState(Character character)
        {
            character.CancelAllActions();
            character.IsFighting = false;
            character.IsMoving = false;
            character.IsWaitingForAtackAnimation = false;
            character.CurrentMovingVector = Vector2.Zero;
            character.LastNewPosition = character.Asda2Position;
            character.HasPendingChargeAnimationPath = false;
        }
        ...
        public static void PlayerLoginRequestLS(IRealmClient client, RealmPacketIn packet)
        {
            ...
            else
            {
                ...
                packet.Position += 2;
                var chanel = packet.ReadByte();
                if (chanel >= ChannelCount)
                {
                    client.Disconnect(false);
                    return;
                }
                ...
                {
                    var characterRecord = client.Account.Characters.FirstOrDefault(chr => chr.CharNum == num1);
                    if (characterRecord == null)
                        client.Disconnect(false);
                    else
                        PreLoginCharacter(client, characterRecord.EntityLowId, true, chanel);
                }
            }
        }
        ...
        {
            PreLoginCharacter(client, charLowId, isLoginStep, client == null ? (byte)0 : client.Channel);
        }

        public static void PreLoginCharacter(IRealmClient client, uint charLowId, bool isLoginStep, byte channel)
        {
            try
            {
                if (client == null)
        ...
        private static void LoginCharacter(IRealmClient client, uint charLowId, bool isLoginStep)
        {
            ...
            if (account == null)
                return;
            CharacterRecord record = ResolveCharacterRecord(account, charLowId);
            if (record == null)
            {
        ...
        private static CharacterRecord ResolveCharacterRecord(RealmAccount account, uint charLowId)
        {
            if (account == null)
                return null;

            var record = account.GetCharacterRecord(charLowId);
            if (record != null)
                return record;

            var legacyOffset = (long)charLowId - account.AccountId;
            if (legacyOffset <= 0 || legacyOffset % 1000000 != 0)
                return null;

            var charNum = (byte)(legacyOffset / 1000000);
            return account.Characters.FirstOrDefault(chr => chr.CharNum == charNum);
        }
        ...
        public static void SendEnterGameResposeResponse(IRealmClient client)
        {
            ...
            using (RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.EnterGameRespose))
            {
                ...
                packet.WriteByte((byte)activeCharacter.Class);
                ...
                packet.WriteInt16(activeCharacter.ProfessionLevel);
                packet.WriteByte(0);
                packet.WriteInt16(activeCharacter.MaxWeight);
                packet.WriteInt16(activeCharacter.Asda2Inventory.Weight);//Weight
        ...
        public static void SendEnterGameResponseItemsOnCharacterResponse(IRealmClient client)
        {
            using (RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.EnterGameResponseItemsOnCharacter))
            {
                ...
                if (client.Locale == Locale.Tahadi)
                    packet.WriteInt32(0);
                ...
                for (int index = 0; index < 12; ++index)
                {
                    ...
                    packet.WriteInt32(0);
                    packet.WriteByte(asda2Item == null ? 0 : asda2Item.Durability);
                    packet.WriteInt16(0);
                    ...
                    packet.WriteInt16(asda2Item == null ? 0 : asda2Item.Enchant);
                    packet.WriteInt16(0);
                    packet.WriteByte(0);
        ...
        public static void SendEnterWorldIpeResponseResponse(IRealmClient client)
        {
            using (RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.EnterWorldIpeResponse))
            {
                ...
                packet.WriteInt16(Convert.ToInt16(client.ActiveCharacter.Asda2X));
                packet.WriteInt16(Convert.ToInt16(client.ActiveCharacter.Asda2Y));
        ...
        public static void CharacterInitOnLoginRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            {
                packet.Position += 4;
                uint requestedLegacyCharId = Character.LegacyCharacterIdFromAccIdAndCharNum(num1, num2);
                ...
                else
                {
                    ...
                    else
                    {
                        CharacterRecord characterRecord =
                            loggedInAccount.Characters.FirstOrDefault(chr => chr.CharNum == (byte)num2);
                        if (characterRecord == null)
                        {
                            client.Disconnect(false);
                            return;
                        ...
                        uint charLowId = characterRecord.EntityLowId;
                        ...
                        Log.Create(Log.Types.AccountOperations, LogSourceType.Account, (uint)num1)
                          ...
                          .AddAttribute("character", client.Account.ActiveCharacter.EntryId,
                            client.Account.ActiveCharacter.Name).AddAttribute("chrLowId", charLowId, "")
                          .AddAttribute("requestedLegacyChrLowId", requestedLegacyCharId, "")
                          .Write();
                        Log.Create(Log.Types.AccountOperations, LogSourceType.Character, charLowId)
                          .AddAttribute("operation", 1.0, "login_game_server")
                          .AddAttribute("ip", 0.0, client.ClientAddress.ToString()).Write();
                        PreLoginCharacter(client, charLowId, false, GetActiveCharacterChannel(loggedInAccount));
                    }
                }
            }
        }
            ...
            else
            {
                ...
                else
                {
                    ...
                    PreLoginCharacter(client, loggedInAccount.ActiveCharacter.EntityId.Low, false,
                        GetActiveCharacterChannel(loggedInAccount));
                }
            }
        }

        private static byte GetActiveCharacterChannel(RealmAccount account)
        {
            if (account == null || account.ActiveCharacter == null || account.ActiveCharacter.Client == null)
                return 0;
            byte channel = account.ActiveCharacter.Client.Channel;
            return channel < ChannelCount ? channel : (byte)0;
        }
